 ...
 */
(function(exports) {
    var cString = window.String, cBoolean = window.Boolean, cNumber = window.Number, cObject = window.Object, cArray = window.Array, cRegExp = window.RegExp, cDate = window.Date, cFunction = window.Function, cMath = window.Math, cError = window.Error, cSyntaxError = window.SyntaxError, cTypeError = window.TypeError, fIsNaN = window.isNaN, fIsFinite = window.isFinite, nNaN = window.NaN, nInfinity = window.Infinity, fWindow_btoa = window.btoa, fWindow_atob = window.atob, fWindow_parseInt = window.parseInt, fString_trim = function() {
        ...
        return cArray.prototype.indexOf ? function(aValue, vItem) {
            ...
            for (var nIndex = 0, nLength = aValue.length; nIndex < nLength; nIndex++) if (aValue[nIndex] === vItem) return nIndex;
            return -1;
        };
    }();
    var sNS_XSD = "http://www.w3.org/2001/XMLSchema", sNS_XPF = "http://www.w3.org/2005/xpath-functions", sNS_XNS = "http://www.w3.org/2000/xmlns/", sNS_XML = "http://www.w3.org/XML/1998/namespace";
    ...
    function cLexer(sValue) {
        ...
        if (aMatch) {
            var nStack = 0;
            for (var nIndex = 0, nLength = aMatch.length; nIndex < nLength; nIndex++) if (aMatch[nIndex] == "(:") nStack++; else if (aMatch[nIndex] == ":)" && nStack) nStack--; else if (!nStack && !/^\s/.test(aMatch[nIndex])) this[this.length++] = aMatch[nIndex];
    ...
    function cDynamicContext(oStaticContext, vItem, oScope, oDOMAdapter) {
        ...
        var oDate = new cDate(), nOffset = oDate.getTimezoneOffset();
        this.dateTime = new cXSDateTime(oDate.getFullYear(), oDate.getMonth() + 1, oDate.getDate(), oDate.getHours(), oDate.getMinutes(), oDate.getSeconds() + oDate.getMilliseconds() / 1e3, -nOffset);
    ...
    cStaticContext.prototype.getDataType = function(sUri) {
        var aMatch = sUri.match(rStaticContext_uri);
        if (aMatch) return aMatch[1] == sNS_XSD ? hStaticContext_dataTypes[aMatch[2]] : this.dataTypes[sUri];
    };
    ...
    cStaticContext.prototype.getFunction = function(sUri) {
        var aMatch = sUri.match(rStaticContext_uri);
        if (aMatch) return aMatch[1] == sNS_XPF ? hStaticContext_functions[aMatch[2]] : this.functions[sUri];
    };
    ...
    cStaticContext.prototype.getURIForPrefix = function(sPrefix) {
        var oResolver = this.namespaceResolver, fResolver = oResolver && oResolver.lookupNamespaceURI ? oResolver.lookupNamespaceURI : oResolver, sNameSpaceURI;
    ...
    var hStaticContext_functions = {}, hStaticContext_signatures = {}, hStaticContext_dataTypes = {}, hStaticContext_operators = {};
    ...
    function cExpression(sExpression, oStaticContext) {
        var oLexer = new cLexer(sExpression), oExpr = fExpr_parse(oLexer, oStaticContext);
    ...
    cExpr.prototype.evaluate = function(oContext) {
        var oSequence = [];
        for (var nIndex = 0, nLength = this.items.length; nIndex < nLength; nIndex++) oSequence = hStaticContext_operators["concatenate"].call(oContext, oSequence, this.items[nIndex].evaluate(oContext));
        return oSequence;
    };
    ...
    function fForExpr_parse(oLexer, oStaticContext) {
        if (oLexer.peek() == "for" && oLexer.peek(1).substr(0, 1) == "$") {
            oLexer.next();
            var oForExpr = new cForExpr(), oExpr;
            do {
    ...
    cForExpr.prototype.evaluate = function(oContext) {
        ...
        (function(oSelf, nBinding) {
            var oBinding = oSelf.bindings[nBinding++], oSequence1 = oBinding.inExpr.evaluate(oContext), sUri = (oBinding.namespaceURI ? "{" + oBinding.namespaceURI + "}" : "") + oBinding.localName;
    ...
    function fQuantifiedExpr_parse(oLexer, oStaticContext) {
        ...
        if ((sQuantifier == "some" || sQuantifier == "every") && oLexer.peek(1).substr(0, 1) == "$") {
            oLexer.next();
            var oQuantifiedExpr = new cQuantifiedExpr(sQuantifier), oExpr;
            do {
    ...
    cQuantifiedExpr.prototype.evaluate = function(oContext) {
        var bEvery = this.quantifier == "every", bResult = bEvery ? true : false;
        (function(oSelf, nBinding) {
            var oBinding = oSelf.bindings[nBinding++], oSequence1 = oBinding.inExpr.evaluate(oContext), sUri = (oBinding.namespaceURI ? "{" + oBinding.namespaceURI + "}" : "") + oBinding.localName;
    ...
    function fComparisonExpr_ValueComp(oExpr, oContext) {
        ...
        var vLeft = oLeft[0], vRight = oRight[0];
    ...
    function fAdditiveExpr_parse(oLexer, oStaticContext) {
        ...
        var oAdditiveExpr = new cAdditiveExpr(oExpr), sOperator;
    ...
    function fMultiplicativeExpr_parse(oLexer, oStaticContext) {
        ...
        var oMultiplicativeExpr = new cMultiplicativeExpr(oExpr), sOperator;
    ...
    cOrExpr.prototype.evaluate = function(oContext) {
        ...
        for (var nIndex = 0, nLength = this.items.length; nIndex < nLength && !bValue; nIndex++) bValue = fFunction_sequence_toEBV(this.items[nIndex].evaluate(oContext), oContext);
    ...
    cStepExpr.prototype.applyPredicates = function(oSequence, oContext) {
        var vContextItem = oContext.item, nContextPosition = oContext.position, nContextSize = oContext.size;
    ...
    function fAxisStep_parse(oLexer, oStaticContext) {
        var sAxis = oLexer.peek(), oExpr, oStep;
        if (oLexer.peek(1) == "::") {
    ...
    cAxisStep.prototype.evaluate = function(oContext) {
        ...
        var oSequence = [], fGetProperty = oContext.DOMAdapter.getProperty, nType = fGetProperty(oItem, "nodeType");
        switch (this.axis) {
          case "attribute":
            if (nType == 1) for (var aAttributes = fGetProperty(oItem, "attributes"), nIndex = 0, nLength = aAttributes.length; nIndex < nLength; nIndex++) oSequence.push(aAttributes[nIndex]);
            break;

          case "child":
            for (var oNode = fGetProperty(oItem, "firstChild"); oNode; oNode = fGetProperty(oNode, "nextSibling")) oSequence.push(oNode);
            break;
          ...
          case "following":
            for (var oParent = oItem, oSibling; oParent; oParent = fGetProperty(oParent, "parentNode")) if (oSibling = fGetProperty(oParent, "nextSibling")) fAxisStep_getChildrenForward(oSibling, oSequence, fGetProperty);
            break;

          case "following-sibling":
            for (var oNode = oItem; oNode = fGetProperty(oNode, "nextSibling"); ) oSequence.push(oNode);
            break;
          ...
          case "ancestor":
            for (var oNode = nType == 2 ? fGetProperty(oItem, "ownerElement") : oItem; oNode = fGetProperty(oNode, "parentNode"); ) oSequence.push(oNode);
            break;
          ...
          case "preceding":
            for (var oParent = oItem, oSibling; oParent; oParent = fGetProperty(oParent, "parentNode")) if (oSibling = fGetProperty(oParent, "previousSibling")) fAxisStep_getChildrenBackward(oSibling, oSequence, fGetProperty);
            break;

          case "preceding-sibling":
            for (var oNode = oItem; oNode = fGetProperty(oNode, "previousSibling"); ) oSequence.push(oNode);
            break;
        }
    ...
    function fPathExpr_parse(oLexer, oStaticContext) {
        if (oLexer.eof()) return;
        var sSingleSlash = "/", sDoubleSlash = "/" + "/";
        var oPathExpr = new cPathExpr(), sSlash = oLexer.peek(), oExpr;
        ...
        if (oLexer.eof() || !(oExpr = fStepExpr_parse(oLexer, oStaticContext))) {
            if (sSlash == sSingleSlash) return oPathExpr.items[0];
            if (sSlash == sDoubleSlash) throw new cException("XPST0003", "Expected path step expression");
            return;
        }
    ...
    cPathExpr.prototype.evaluate = function(oContext) {
        ...
        for (var nItemIndex = 0, nItemLength = this.items.length, oSequence1; nItemIndex < nItemLength; nItemIndex++) {
            ...
            for (var nIndex = 0, nLength = oSequence.length; nIndex < nLength; nIndex++) {
                ...
                for (var nRightIndex = 0, oSequence2 = this.items[nItemIndex].evaluate(oContext), nRightLength = oSequence2.length; nRightIndex < nRightLength; nRightIndex++) if (nItemIndex < nItemLength - 1 && !oContext.DOMAdapter.isNode(oSequence2[nRightIndex])) throw new cException("XPTY0019"); else if (fArray_indexOf(oSequence1, oSequence2[nRightIndex]) == -1) oSequence1.push(oSequence2[nRightIndex]);
            }
    ...
    function fKindTest_parse(oLexer, oStaticContext) {
        var sName = oLexer.peek(), oValue;
        if (oLexer.peek(1) == "(") {
            if (!(sName in hKindTest_names)) throw new cException("XPST0003", "Unknown '" + sName + "' kind test");
            oLexer.next(2);
            var oTest = new cKindTest(sName);
            if (oLexer.peek() != ")") {
                if (sName == "document-node") {} else if (sName == "element") {} else if (sName == "attribute") {} else if (sName == "processing-instruction") {
                ...
                } else if (sName == "schema-attribute") {} else if (sName == "schema-element") {}
            } else {
    ...
    cKindTest.prototype.test = function(oNode, oContext) {
        var fGetProperty = oContext.DOMAdapter.getProperty, nType = oContext.DOMAdapter.isNode(oNode) ? fGetProperty(oNode, "nodeType") : 0, sTarget;
        switch (this.name) {
          case "node":
            return !!nType;
    ...
    cNameTest.prototype.test = function(oNode, oContext) {
        var fGetProperty = oContext.DOMAdapter.getProperty, nType = fGetProperty(oNode, "nodeType");
        if (nType == 1 || nType == 2) {
    ...
    function fNumericLiteral_parse(oLexer, oStaticContext) {
        var sValue = oLexer.peek(), vValue = fNumericLiteral_parseValue(sValue);
        if (vValue) {
            oLexer.next();
    ...
    function fFunctionCall_parse(oLexer, oStaticContext) {
        ...
        if (aMatch && oLexer.peek(1) == "(") {
            ...
            var oFunctionCallExpr = new cFunctionCall(aMatch[1] || null, aMatch[2], aMatch[1] ? oStaticContext.getURIForPrefix(aMatch[1]) || null : oStaticContext.defaultFunctionNamespace), oExpr;
            oLexer.next(2);
            if (oLexer.peek() != ")") {
                do {
    ...
    cFunctionCall.prototype.evaluate = function(oContext) {
        var aArguments = [], aParameters, fFunction;
        for (var nIndex = 0, nLength = this.args.length; nIndex < nLength; nIndex++) aArguments.push(this.args[nIndex].evaluate(oContext));
        var sUri = (this.namespaceURI ? "{" + this.namespaceURI + "}" : "") + this.localName;
    ...
    function fFunctionCall_prepare(sName, aParameters, fFunction, aArguments, oContext) {
        var oArgument, nArgumentsLength = aArguments.length, oParameter, nParametersLength = aParameters.length, nParametersRequired = 0;
        while (nParametersRequired < aParameters.length && !aParameters[nParametersRequired][2]) nParametersRequired++;
    ...
    cIntersectExceptExpr.prototype.evaluate = function(oContext) {
        ...
        for (var nIndex = 0, nLength = this.items.length, oItem; nIndex < nLength; nIndex++) oSequence = hStaticContext_operators[(oItem = this.items[nIndex])[0]].call(oContext, oSequence, oItem[1].evaluate(oContext));
        return oSequence;
    };
    ...
    cUnionExpr.prototype.evaluate = function(oContext) {
        var oSequence = this.left.evaluate(oContext);
        for (var nIndex = 0, nLength = this.items.length; nIndex < nLength; nIndex++) oSequence = hStaticContext_operators["union"].call(oContext, oSequence, this.items[nIndex].evaluate(oContext));
        return oSequence;
    };
    ...
    cInstanceofExpr.prototype.evaluate = function(oContext) {
        var oSequence1 = this.expression.evaluate(oContext), oItemType = this.type.itemType, sOccurence = this.type.occurence;
        ...
        var bValue = true;
        for (var nIndex = 0, nLength = oSequence1.length; nIndex < nLength && bValue; nIndex++) bValue = oItemType.test.test(oSequence1[nIndex], oContext);
        return [ new cXSBoolean(bValue) ];
    };
    ...
    cTreatExpr.prototype.evaluate = function(oContext) {
        ...
        for (var nIndex = 0, nLength = oSequence1.length; nIndex < nLength; nIndex++) if (!oItemType.test.test(oSequence1[nIndex], oContext)) throw new cException("XPDY0050", "Required item type of value in 'treat as' expression is " + (oItemType.test.prefix ? oItemType.test.prefix + ":" : "") + oItemType.test.localName);
        return oSequence1;
    };
    ...
    cAtomicType.prototype.test = function(vItem, oContext) {
        var sUri = (this.namespaceURI ? "{" + this.namespaceURI + "}" : "") + this.localName, cType = this.namespaceURI == sNS_XSD ? hStaticContext_dataTypes[this.localName] : oContext.staticContext.getDataType(sUri);
    ...
    cXSBase64Binary.cast = function(vValue) {
        ...
        if (vValue instanceof cXSHexBinary) {
            var aMatch = vValue.valueOf().match(/.{2}/g), aValue = [];
            for (var nIndex = 0, nLength = aMatch.length; nIndex < nLength; nIndex++) aValue.push(cString.fromCharCode(fWindow_parseInt(aMatch[nIndex], 16)));
    ...
    cXSDate.cast = function(vValue) {
        ...
        if (vValue instanceof cXSString || vValue instanceof cXSUntypedAtomic) {
            ...
            if (aMatch) {
                var nYear = +aMatch[2], nMonth = +aMatch[3], nDay = +aMatch[4];
    ...
    cXSDateTime.cast = function(vValue) {
        ...
        if (vValue instanceof cXSString || vValue instanceof cXSUntypedAtomic) {
            ...
            if (aMatch) {
                var nYear = +aMatch[2], nMonth = +aMatch[3], nDay = +aMatch[4], bValue = !!aMatch[10];
    ...
    cXSDecimal.cast = function(vValue) {
        ...
        if (fXSAnyAtomicType_isNumeric(vValue)) {
            if (!fIsNaN(vValue) && fIsFinite(vValue)) return new cXSDecimal(+vValue);
    ...
    cXSGMonthDay.cast = function(vValue) {
        ...
        if (vValue instanceof cXSString || vValue instanceof cXSUntypedAtomic) {
            ...
            if (aMatch) {
                var nMonth = +aMatch[1], nDay = +aMatch[2];
    ...
    cXSGYearMonth.cast = function(vValue) {
        ...
        if (vValue instanceof cXSString || vValue instanceof cXSUntypedAtomic) {
            ...
            if (aMatch) {
                var nYear = +aMatch[1], nMonth = +aMatch[2];
    ...
    cXSHexBinary.cast = function(vValue) {
        ...
        if (vValue instanceof cXSBase64Binary) {
            var sValue = fWindow_atob(vValue.valueOf()), aValue = [];
    ...
    cXSInteger.cast = function(vValue) {
        if (vValue instanceof cXSInteger) return new cXSInteger(vValue.value);
        if (vValue instanceof cXSString || vValue instanceof cXSUntypedAtomic) {
        ...
        if (fXSAnyAtomicType_isNumeric(vValue)) {
            if (!fIsNaN(vValue) && fIsFinite(vValue)) return new cXSInteger(+vValue);
    ...
    cXSNonPositiveInteger.cast = function(vValue) {
        ...
        if (oValue.value <= 0) return new cXSNonPositiveInteger(oValue.value);
        throw new cException("FORG0001");
    };
    ...
    cXSNegativeInteger.cast = function(vValue) {
        ...
        if (oValue.value <= -1) return new cXSNegativeInteger(oValue.value);
        throw new cException("FORG0001");
    };
    ...
    cXSLong.cast = function(vValue) {
        ...
        if (oValue.value <= 0x8000000000000000 && oValue.value >= -0x8000000000000000) return new cXSLong(oValue.value);
        throw new cException("FORG0001");
    };
    ...
    cXSInt.cast = function(vValue) {
        ...
        if (oValue.value <= 2147483647 && oValue.value >= -2147483648) return new cXSInt(oValue.value);
        throw new cException("FORG0001");
    };
    ...
    cXSShort.cast = function(vValue) {
        ...
        if (oValue.value <= 32767 && oValue.value >= -32768) return new cXSShort(oValue.value);
        throw new cException("FORG0001");
    };
    ...
    cXSByte.cast = function(vValue) {
        ...
        if (oValue.value <= 127 && oValue.value >= -128) return new cXSByte(oValue.value);
        throw new cException("FORG0001");
    };
    ...
    cXSNonNegativeInteger.cast = function(vValue) {
        ...
        if (oValue.value >= 0) return new cXSNonNegativeInteger(oValue.value);
        throw new cException("FORG0001");
    };
    ...
    cXSPositiveInteger.cast = function(vValue) {
        ...
        if (oValue.value >= 1) return new cXSPositiveInteger(oValue.value);
        throw new cException("FORG0001");
    };
    ...
    cXSUnsignedLong.cast = function(vValue) {
        ...
        if (oValue.value >= 1 && oValue.value <= 0x10000000000000000) return new cXSUnsignedLong(oValue.value);
        throw new cException("FORG0001");
    };
    ...
    cXSUnsignedInt.cast = function(vValue) {
        ...
        if (oValue.value >= 1 && oValue.value <= 4294967295) return new cXSUnsignedInt(oValue.value);
        throw new cException("FORG0001");
    };
    ...
    cXSUnsignedShort.cast = function(vValue) {
        ...
        if (oValue.value >= 1 && oValue.value <= 65535) return new cXSUnsignedShort(oValue.value);
        throw new cException("FORG0001");
    };
    ...
    cXSUnsignedByte.cast = function(vValue) {
        ...
        if (oValue.value >= 1 && oValue.value <= 255) return new cXSUnsignedByte(oValue.value);
        throw new cException("FORG0001");
    };
    ...
    function fOperator_compareTimes(oLeft, oRight, sComparator) {
        var nLeft = fOperator_time_toSeconds(oLeft), nRight = fOperator_time_toSeconds(oRight);
    ...
    function fOperator_compareDateTimes(oLeft, oRight, sComparator) {
        var oTimezone = new cXSDayTimeDuration(0, 0, 0, 0), sLeft = fFunction_dateTime_adjustTimezone(oLeft, oTimezone).toString(), sRight = fFunction_dateTime_adjustTimezone(oRight, oTimezone).toString();
    ...
    function fOperator_dayTimeDuration_fromSeconds(nValue) {
        var bNegative = (nValue = cMath.round(nValue)) < 0, nDays = ~~((nValue = cMath.abs(nValue)) / 86400), nHours = ~~((nValue -= nDays * 3600 * 24) / 3600), nMinutes = ~~((nValue -= nHours * 3600) / 60), nSeconds = nValue -= nMinutes * 60;
    ...
    function fOperator_yearMonthDuration_fromMonths(nValue) {
        var nNegative = (nValue = cMath.round(nValue)) < 0, nYears = ~~((nValue = cMath.abs(nValue)) / 12), nMonths = nValue -= nYears * 12;
    ...
    function fFunctionCall_numeric_getPower(oLeft, oRight) {
        ...
        var aLeft = cString(oLeft).match(rNumericLiteral), aRight = cString(oRight).match(rNumericLiteral), nPower = cMath.max(1, (aLeft[2] || aLeft[3] || "").length + (aLeft[5] || 0) * (aLeft[4] == "+" ? -1 : 1), (aRight[2] || aRight[3] || "").length + (aRight[5] || 0) * (aRight[4] == "+" ? -1 : 1));
        return nPower + (nPower % 2 ? 0 : 1);
    }
    hStaticContext_operators["numeric-add"] = function(oLeft, oRight) {
        var nLeft = oLeft.valueOf(), nRight = oRight.valueOf(), nPower = cMath.pow(10, fFunctionCall_numeric_getPower(nLeft, nRight));
    ...
    hStaticContext_operators["union"] = function(oSequence1, oSequence2) {
        ...
        for (var nIndex = 0, nLength = oSequence1.length, oItem; nIndex < nLength; nIndex++) {
            if (!this.DOMAdapter.isNode(oItem = oSequence1[nIndex])) throw new cException("XPTY0004", "Required item type of first operand of 'union' is node()");
            if (fArray_indexOf(oSequence, oItem) == -1) oSequence.push(oItem);
        }
        for (var nIndex = 0, nLength = oSequence2.length, oItem; nIndex < nLength; nIndex++) {
            if (!this.DOMAdapter.isNode(oItem = oSequence2[nIndex])) throw new cException("XPTY0004", "Required item type of second operand of 'union' is node()");
            if (fArray_indexOf(oSequence, oItem) == -1) oSequence.push(oItem);
        }
    ...
    hStaticContext_operators["intersect"] = function(oSequence1, oSequence2) {
        ...
        for (var nIndex = 0, nLength = oSequence1.length, oItem, bFound; nIndex < nLength; nIndex++) {
            if (!this.DOMAdapter.isNode(oItem = oSequence1[nIndex])) throw new cException("XPTY0004", "Required item type of second operand of 'intersect' is node()");
            bFound = false;
    ...
    hStaticContext_operators["except"] = function(oSequence1, oSequence2) {
        var oSequence = [];
        for (var nIndex = 0, nLength = oSequence1.length, oItem, bFound; nIndex < nLength; nIndex++) {
            if (!this.DOMAdapter.isNode(oItem = oSequence1[nIndex])) throw new cException("XPTY0004", "Required item type of second operand of 'except' is node()");
            bFound = false;
            for (var nRightIndex = 0, nRightLength = oSequence2.length; nRightIndex < nRightLength && !bFound; nRightIndex++) {
    ...
    hStaticContext_operators["to"] = function(oLeft, oRight) {
        var oSequence = [];
        for (var nIndex = oLeft.valueOf(), nLength = oRight.valueOf(); nIndex <= nLength; nIndex++) oSequence.push(new cXSInteger(nIndex));
        return oSequence;
    };
    ...
    fStaticContext_defineSystemFunction("resolve-uri", [ [ cXSString, "?" ], [ cXSString, "", true ] ], function(sUri, sBaseUri) {
        ...
        if (!oUri.authority) {
            ...
            if (oUri.path.charAt(0) != "/") {
                var aUriSegments = oUri.path.split("/"), aBaseUriSegments = oBaseUri.path.split("/");
    ...
    fStaticContext_defineSystemFunction("root", [ [ cXTNode, "?", true ] ], function(oNode) {
        ...
        var fGetProperty = this.DOMAdapter.getProperty;
        if (fGetProperty(oNode, "nodeType") == 2) oNode = fGetProperty(oNode, "ownerElement");
        for (var oParent = oNode; oParent; oParent = fGetProperty(oNode, "parentNode")) oNode = oParent;
        return oNode;
    });
    ...
    fStaticContext_defineSystemFunction("round-half-to-even", [ [ cXSDouble, "?" ], [ cXSInteger, "", true ] ], function(oValue, oPrecision) {
        ...
        if (nPrecision < 0) {
            var oPower = new cXSInteger(cMath.pow(10, -nPrecision)), nRounded = cMath.round(hStaticContext_operators["numeric-divide"].call(this, oValue, oPower)), oRounded = new cXSInteger(nRounded);
            ...
            return hStaticContext_operators["numeric-multiply"].call(this, hStaticContext_operators["numeric-add"].call(this, oRounded, new cXSDecimal(nDecimal == .5 && nRounded % 2 ? -1 : 0)), oPower);
        } else {
            var oPower = new cXSInteger(cMath.pow(10, nPrecision)), nRounded = cMath.round(hStaticContext_operators["numeric-multiply"].call(this, oValue, oPower)), oRounded = new cXSInteger(nRounded);
            ...
            return hStaticContext_operators["numeric-divide"].call(this, hStaticContext_operators["numeric-add"].call(this, oRounded, new cXSDecimal(nDecimal == .5 && nRounded % 2 ? -1 : 0)), oPower);
        }
    });
    fStaticContext_defineSystemFunction("resolve-QName", [ [ cXSString, "?" ], [ cXTElement ] ], function(oQName, oElement) {
        ...
        var sQName = oQName.valueOf(), aMatch = sQName.match(rXSQName);
        if (!aMatch) throw new cException("FOCA0002", "Invalid QName '" + sQName + "'");
        var sPrefix = aMatch[1] || null, sLocalName = aMatch[2], sNameSpaceURI = this.DOMAdapter.lookupNamespaceURI(oElement, sPrefix);
    ...
    fStaticContext_defineSystemFunction("namespace-uri-for-prefix", [ [ cXSString, "?" ], [ cXTElement ] ], function(oPrefix, oElement) {
        var sPrefix = oPrefix == null ? "" : oPrefix.valueOf(), sNameSpaceURI = this.DOMAdapter.lookupNamespaceURI(oElement, sPrefix || null);
    ...
    fStaticContext_defineSystemFunction("distinct-values", [ [ cXSAnyAtomicType, "*" ], [ cXSString, "", true ] ], function(oSequence1, oCollation) {
        if (arguments.length > 1) throw "Collation parameter in function '" + "distinct-values" + "' is not implemented";
    ...
    fStaticContext_defineSystemFunction("insert-before", [ [ cXTItem, "*" ], [ cXSInteger ], [ cXTItem, "*" ] ], function(oSequence1, oPosition, oSequence3) {
        ...
        var nLength = oSequence1.length, nPosition = oPosition.valueOf();
    ...
    fStaticContext_defineSystemFunction("remove", [ [ cXTItem, "*" ], [ cXSInteger ] ], function(oSequence1, oPosition) {
        ...
        var oSequence = [];
        for (var nIndex = 0; nIndex < nLength; nIndex++) if (nPosition != nIndex + 1) oSequence.push(oSequence1[nIndex]);
        return oSequence;
    });
    ...
    fStaticContext_defineSystemFunction("subsequence", [ [ cXTItem, "*" ], [ cXSDouble, "" ], [ cXSDouble, "", true ] ], function(oSequence1, oStart, oLength) {
        var nPosition = cMath.round(oStart), nLength = arguments.length > 2 ? cMath.round(oLength) : oSequence1.length - nPosition + 1;
    ...
    fStaticContext_defineSystemFunction("max", [ [ cXSAnyAtomicType, "*" ], [ cXSString, "", true ] ], function(oSequence1, oCollation) {
        ...
        try {
            ...
            for (var nIndex = 1, nLength = oSequence1.length, vRight; nIndex < nLength; nIndex++) {
                vRight = oSequence1[nIndex];
                if (vRight instanceof cXSUntypedAtomic) vRight = cXSDouble.cast(vRight);
                if (hComparisonExpr_ValueComp_operators["ge"](vRight, vValue, this).valueOf()) vValue = vRight;
            }
            return vValue;
        } catch (e) {
    ...
    fStaticContext_defineSystemFunction("min", [ [ cXSAnyAtomicType, "*" ], [ cXSString, "", true ] ], function(oSequence1, oCollation) {
        ...
        try {
            ...
            for (var nIndex = 1, nLength = oSequence1.length, vRight; nIndex < nLength; nIndex++) {
                vRight = oSequence1[nIndex];
                if (vRight instanceof cXSUntypedAtomic) vRight = cXSDouble.cast(vRight);
                if (hComparisonExpr_ValueComp_operators["le"](vRight, vValue, this).valueOf()) vValue = vRight;
            }
            return vValue;
        } catch (e) {
    ...
    fStaticContext_defineSystemFunction("id", [ [ cXSString, "*" ], [ cXTNode, "", true ] ], function(oSequence1, oNode) {
        ...
        var oSequence = [];
        for (var nIndex = 0; nIndex < oSequence1.length; nIndex++) for (var nRightIndex = 0, aValue = fString_trim(oSequence1[nIndex]).split(/\s+/), nRightLength = aValue.length; nRightIndex < nRightLength; nRightIndex++) if ((oNode = this.DOMAdapter.getElementById(oDocument, aValue[nRightIndex])) && fArray_indexOf(oSequence, oNode) == -1) oSequence.push(oNode);
        return fFunction_sequence_order(oSequence, this);
    });
    ...
    fStaticContext_defineSystemFunction("codepoints-to-string", [ [ cXSInteger, "*" ] ], function(oSequence1) {
        var aValue = [];
        for (var nIndex = 0, nLength = oSequence1.length; nIndex < nLength; nIndex++) aValue.push(cString.fromCharCode(oSequence1[nIndex]));
    ...
    fStaticContext_defineSystemFunction("string-to-codepoints", [ [ cXSString, "?" ] ], function(oValue) {
        ...
        if (sValue == "") return [];
        var oSequence = [];
        for (var nIndex = 0, nLength = sValue.length; nIndex < nLength; nIndex++) oSequence.push(new cXSInteger(sValue.charCodeAt(nIndex)));
        return oSequence;
    });
    fStaticContext_defineSystemFunction("compare", [ [ cXSString, "?" ], [ cXSString, "?" ], [ cXSString, "", true ] ], function(oValue1, oValue2, oCollation) {
        ...
        var sCollation = this.staticContext.defaultCollationName, vCollation;
    ...
    fStaticContext_defineSystemFunction("substring", [ [ cXSString, "?" ], [ cXSDouble ], [ cXSDouble, "", true ] ], function(oValue, oStart, oLength) {
        var sValue = oValue == null ? "" : oValue.valueOf(), nStart = cMath.round(oStart) - 1, nEnd = arguments.length > 2 ? nStart + cMath.round(oLength) : sValue.length;
    ...
    fStaticContext_defineSystemFunction("translate", [ [ cXSString, "?" ], [ cXSString ], [ cXSString ] ], function(oValue, oMap, oTranslate) {
        ...
        var aValue = oValue.valueOf().split(""), aMap = oMap.valueOf().split(""), aTranslate = oTranslate.valueOf().split(""), nTranslateLength = aTranslate.length, aReturn = [];
        for (var nIndex = 0, nLength = aValue.length, nPosition; nIndex < nLength; nIndex++) if ((nPosition = aMap.indexOf(aValue[nIndex])) == -1) aReturn[aReturn.length] = aValue[nIndex]; else if (nPosition < nTranslateLength) aReturn[aReturn.length] = aTranslate[nPosition];
    ...
    fStaticContext_defineSystemFunction("escape-html-uri", [ [ cXSString, "?" ] ], function(oValue) {
        ...
        for (var nIndex = 0, nLength = aValue.length, nCode; nIndex < nLength; nIndex++) if ((nCode = aValue[nIndex].charCodeAt(0)) < 32 || nCode > 126) aValue[nIndex] = window.encodeURIComponent(aValue[nIndex]);
        return new cXSString(aValue.join(""));
    });
    fStaticContext_defineSystemFunction("contains", [ [ cXSString, "?" ], [ cXSString, "?" ], [ cXSString, "", true ] ], function(oValue, oSearch, oCollation) {
        if (arguments.length > 2) throw "Collation parameter in function '" + "contains" + "' is not implemented";
    ...
    fStaticContext_defineSystemFunction("starts-with", [ [ cXSString, "?" ], [ cXSString, "?" ], [ cXSString, "", true ] ], function(oValue, oSearch, oCollation) {
        if (arguments.length > 2) throw "Collation parameter in function '" + "starts-with" + "' is not implemented";
        return new cXSBoolean((oValue == null ? "" : oValue.valueOf()).indexOf(oSearch == null ? "" : oSearch.valueOf()) == 0);
    });
    fStaticContext_defineSystemFunction("ends-with", [ [ cXSString, "?" ], [ cXSString, "?" ], [ cXSString, "", true ] ], function(oValue, oSearch, oCollation) {
        if (arguments.length > 2) throw "Collation parameter in function '" + "ends-with" + "' is not implemented";
        var sValue = oValue == null ? "" : oValue.valueOf(), sSearch = oSearch == null ? "" : oSearch.valueOf();
    ...
    fStaticContext_defineSystemFunction("substring-before", [ [ cXSString, "?" ], [ cXSString, "?" ], [ cXSString, "", true ] ], function(oValue, oSearch, oCollation) {
        if (arguments.length > 2) throw "Collation parameter in function '" + "substring-before" + "' is not implemented";
        var sValue = oValue == null ? "" : oValue.valueOf(), sSearch = oSearch == null ? "" : oSearch.valueOf(), nPosition;
    ...
    fStaticContext_defineSystemFunction("substring-after", [ [ cXSString, "?" ], [ cXSString, "?" ], [ cXSString, "", true ] ], function(oValue, oSearch, oCollation) {
        if (arguments.length > 2) throw "Collation parameter in function '" + "substring-after" + "' is not implemented";
        var sValue = oValue == null ? "" : oValue.valueOf(), sSearch = oSearch == null ? "" : oSearch.valueOf(), nPosition;
    ...
    function fFunction_string_createRegExp(sValue, sFlags) {
        var d1 = "À-ÖØ-öø-˿", d2 = "Ͱ-ͽͿ-῿‌-‍", d3 = "⁰-↏Ⰰ-⿯、-퟿豈-﷏ﷰ-�", c = "A-Z_a-z\\-.0-9·" + d1 + "̀-ͯ" + d2 + "‿-⁀" + d3, i = "A-Z_a-z" + d1 + d2 + d3;
        ...
        var bFlagS = sFlags.indexOf("s") >= 0, bFlagX = sFlags.indexOf("x") >= 0;
        if (bFlagS || bFlagX) {
    ...
    fStaticContext_defineSystemFunction("matches", [ [ cXSString, "?" ], [ cXSString ], [ cXSString, "", true ] ], function(oValue, oPattern, oFlags) {
        var sValue = oValue == null ? "" : oValue.valueOf(), rRegExp = fFunction_string_createRegExp(oPattern.valueOf(), arguments.length > 2 ? oFlags.valueOf() : "");
    ...
    fStaticContext_defineSystemFunction("replace", [ [ cXSString, "?" ], [ cXSString ], [ cXSString ], [ cXSString, "", true ] ], function(oValue, oPattern, oReplacement, oFlags) {
        var sValue = oValue == null ? "" : oValue.valueOf(), rRegExp = fFunction_string_createRegExp(oPattern.valueOf(), arguments.length > 3 ? oFlags.valueOf() : "");
    ...
    fStaticContext_defineSystemFunction("tokenize", [ [ cXSString, "?" ], [ cXSString ], [ cXSString, "", true ] ], function(oValue, oPattern, oFlags) {
        var sValue = oValue == null ? "" : oValue.valueOf(), rRegExp = fFunction_string_createRegExp(oPattern.valueOf(), arguments.length > 2 ? oFlags.valueOf() : "");
        var oSequence = [];
        for (var nIndex = 0, aValue = sValue.split(rRegExp), nLength = aValue.length; nIndex < nLength; nIndex++) oSequence.push(new cXSString(aValue[nIndex]));
        return oSequence;
    });
    ...
    cEvaluator.prototype.evaluate = function(sExpression, vItem, oStaticContext, oScope, oDOMAdapter) {
        if (!oStaticContext) oStaticContext = this.defaultStaticContext;
        if (!oDOMAdapter) oDOMAdapter = this.defaultDOMAdapter;
        if (typeof vItem == "undefined") vItem = null;
        var oXSScope = {}, oValue;
        if (typeof oScope == "object") for (var sKey in oScope) {
            oValue = oScope[sKey];
            if (oScope.hasOwnProperty(sKey) && oValue != null) oXSScope[sKey] = oDOMAdapter.isNode(oValue) ? oValue : cStaticContext.js2xs(oValue);
        }
        ...
        var oSequence = this.compile(sExpression, oStaticContext).evaluate(oContext), aReturn = [];
        for (var nIndex = 0, nLength = oSequence.length, oItem; nIndex < nLength; nIndex++) aReturn[aReturn.length] = oDOMAdapter.isNode(oItem = oSequence[nIndex]) ? oItem : cStaticContext.xs2js(oItem);
        return aReturn;
    };
    var oXPathEvaluator = new cEvaluator(), oXPathClasses = oXPathEvaluator.classes = {};
    ...
    cLXDOMAdapter.prototype.getProperty = function(oNode, sName) {
        ...
        if (sName == "baseURI") {
            var sBaseURI = "", fResolveUri = oLXDOMAdapter_staticContext.getFunction("{" + "http://www.w3.org/2005/xpath-functions" + "}" + "resolve-uri"), cXSString = oLXDOMAdapter_staticContext.getDataType("{" + "http://www.w3.org/2001/XMLSchema" + "}" + "string");
            for (var oParent = oNode, sUri; oParent; oParent = oParent.parentNode) if (oParent.nodeType == 1 && (sUri = oParent.getAttribute("xml:base"))) sBaseURI = fResolveUri(new cXSString(sUri), new cXSString(sBaseURI)).toString();
            return sBaseURI;
        } else if (sName == "textContent") {
            var aText = [];
            (function(oNode) {
                for (var nIndex = 0, oChild; oChild = oNode.childNodes[nIndex]; nIndex++) if (oChild.nodeType == 3 || oChild.nodeType == 4) aText.push(oChild.data); else if (oChild.nodeType == 1 && oChild.firstChild) arguments.callee(oChild);
            })(oNode);
            return aText.join("");
        }
    };
    cLXDOMAdapter.prototype.compareDocumentPosition = function(oNode, oChild) {
        ...
        var oAttr1 = null, oAttr2 = null, aAttributes, oAttr, oElement, nIndex, nLength;
        if (oNode.nodeType == 2) {
            oAttr1 = oNode;
        ...
        var aChain1 = [], nLength1, oNode1, aChain2 = [], nLength2, oNode2;
        if (oAttr1) aChain1.push(oAttr1);
        for (oElement = oNode; oElement; oElement = oElement.parentNode) aChain1.push(oElement);
        if (oAttr2) aChain2.push(oAttr2);
        for (oElement = oChild; oElement; oElement = oElement.parentNode) aChain2.push(oElement);
        if ((oNode.ownerDocument || oNode) != (oChild.ownerDocument || oChild) || aChain1[aChain1.length - 1] != aChain2[aChain2.length - 1]) return 32 | 1;
        for (nIndex = cMath.min(nLength1 = aChain1.length, nLength2 = aChain2.length); nIndex; --nIndex) if ((oNode1 = aChain1[--nLength1]) != (oNode2 = aChain2[--nLength2])) {
            if (oNode1.nodeType == 2) return 4;
            if (oNode2.nodeType == 2) return 2;
            if (!oNode2.nextSibling) return 4;
            if (!oNode1.nextSibling) return 2;
            for (oElement = oNode2.previousSibling; oElement; oElement = oElement.previousSibling) if (oElement == oNode1) return 4;
            return 2;
        }
    ...
    cLXDOMAdapter.prototype.lookupNamespaceURI = function(oNode, sPrefix) {
        ...
        for (;oNode && oNode.nodeType != 9; oNode = oNode.parentNode) if (sPrefix == this.getProperty(oChild, "prefix")) return this.getProperty(oNode, "namespaceURI"); else if (oNode.nodeType == 1) for (var oAttributes = this.getProperty(oNode, "attributes"), nIndex = 0, nLength = oAttributes.length, sName = "xmlns" + ":" + sPrefix; nIndex < nLength; nIndex++) if (this.getProperty(oAttributes[nIndex], "nodeName") == sName) return this.getProperty(oAttributes[nIndex], "value");
        return null;
    };
    cLXDOMAdapter.prototype.getElementsByTagNameNS = function(oNode, sNameSpaceURI, sLocalName) {
        ...
        var aElements = [], bNameSpaceURI = "*" == sNameSpaceURI, bLocalName = "*" == sLocalName;
        (function(oNode) {
            for (var nIndex = 0, oChild; oChild = oNode.childNodes[nIndex]; nIndex++) if (oChild.nodeType == 1) {
 ...
 * Dual licensed under the MIT and GPL licenses.
 *
 * File has been modified by Amazon.com, Inc under MIT license
 */    Xpath2Library.evaluate = function(expression) {
        var oStaticContext = new cStaticContext();
        oStaticContext.namespaceResolver = null;
        oStaticContext.baseURI = document.location.href;
        oStaticContext.defaultFunctionNamespace = sNS_XPF;
        oStaticContext.defaultElementNamespace = "http://www.w3.org/1999/xhtml";
        var oExpression = new cExpression(expression, oStaticContext);
        return oExpression.evaluate(new cDynamicContext(oStaticContext, document, null, new cLXDOMAdapter()));
    };
})(typeof Xpath2Library == "undefined" ? Xpath2Library = {} : Xpath2Library);