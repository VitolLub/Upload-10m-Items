...
amznpubstudio_jQuery = jQuery.noConflict(true); var $ = amznpubstudio_jQuery;
...
//keeping this initialization alive because of backward compat. Need to remove after refactoring
window.amznPs.linkCodesConfigData = {
    'textLink': 'ktl',
    'imageLink': 'kia',
    'productLink': 'kpl',
    "imageOnlyProductLink": 'kil',
    'copyToClipBoard': 'ktc',
    'socialShare': 'ksh'
};
var linkCodes = window.amznPs.linkCodesConfigData;
window.amznPs.refTagsConfigData = {
    'textLink': 'as_li_bk_tl',
    'imageLink': 'as_li_bk_ia',
    'productLink': 'as_li_bk_pl',
    "imageOnlyProductLink": 'as_li_bk_il',
    'copyToClipBoard': 'as_li_bk_tc'
};
...
var utils = {
    ...
    /*
     ...
     provide start and end of selection
     and last character entered by  user
     */
    'getCaretOffset': function (element, _window) {
        var caretOffset = {location: 0, text: ''};
        if (typeof(element.selectionEnd) !== 'undefined') { // in case of textarea
            var _text = element.value;
            return {location: element.selectionEnd, text: _text, element: element};
        }
        if (typeof _window.getSelection != "undefined") { // for content-editable elements
            var sel = _window.getSelection();
            caretOffset.location = sel.focusOffset;
            var _text = sel.focusNode.data;
            caretOffset.text = _text;
            caretOffset.element = sel.focusNode;
        }
        return caretOffset;
    },

    'saveSelection': function (containerEl, composeWindow) {
        var range = composeWindow.getSelection().getRangeAt(0);
    ...
    'restoreSelection': function (containerEl, savedSel, composeWindow) {
        var charIndex = 0, range = composeWindow.document.createRange();
        ...
        var sel = composeWindow.getSelection();
        sel.removeAllRanges();
        sel.addRange(range);
    },
    ...
    'browserMessages': function () {
        var actions = {
            'userAction': function (request) {
                amznPsBmLaunchApp("runApp", null, function () {
                    var metrics = window.amznPs.metrics;
                    var payLoad = {'action': 'runApp'};
            ...
            'setCmsConfigs': function (request) {
                amznPsBmLaunchApp("setConfigs", request);
            },
            'runApp': function (request) {
                amznPsBmLaunchApp("runApp", null);
            },
            'clearStorage': function (request) {
                window.amznPs.browser.initPlatformConfigs();
            },
            'updateTag': function (request) {
                console.log("Update tag " + request.tag);
                window.amznPs.bootStrap.updateTag(request.tag);
            },
            'appStatus': function (request) {
                window.amznPs.bootStrap.setLinkCreation(request.value);
            },
            'attachShortCode' : function(request) {
                if( !window.amznPs.shortCode)  window.amznPs.shortCode = new amzn_ps_bm_shortcode();
                window.amznPs.shortCode.init(request.shortCodeInfo);
                window.amznPs.options = {
                    configFlags: request.configFlags
                }
            }
        };
...
function amzn_ps_bm_link() {
    this.parLevelsAnchorCheck = 5;
    this.linkTypeToCssClassMap = {textLink:'amzn_ps_bm_tl',imageLink:'amzn_ps_bm_il',imageOnlyProductLink:'amzn_ps_bm_ipl',productLink:'amzn_ps_bm_pl'};
    var productLinkMask = null;

    this.setProductLinkMask = function(_mask) {
      productLinkMask = _mask;
    };

    this.removeproductLinkMask = function(){
        if(productLinkMask) {
            productLinkMask.remove();
            productLinkMask = null;
        }
    };

    this.createProductLinkMask = function(link,callback) {
        var $=amznpubstudio_jQuery;

        var $mask = link.parent().find('#amznps_ed_link_mask');
        if($mask.length !== 0) $mask.remove();
        $mask= $('<div/>',{
            id:'amznps_ed_link_mask',
            css:{
        ...
        $mask.mouseleave({self:this},function(event){
            var e = event.toElement || event.relatedTarget;
            var self = event.data.self;
            self.removeproductLinkMask();
        });
        this.setProductLinkMask($mask);
        if(callback) callback();
        return $mask;
    };

    this.getProductLinkMask = function(){
        return productLinkMask.get(0);
    };
}

//textNode to green colored span
amzn_ps_bm_link.prototype.replaceTextWithNode = function(text, textNode, startIndex) {
    var self=this;
    var $=amznpubstudio_jQuery;
    ...
    }).addClass("amzn_ps_bm_text_span_under_save amzn_ps_bm_text_span").text(text).insertBefore(textNode);
    if(startIndex+text.length<content.length) {
        $(DOCUMENT.createTextNode(content.substring(startIndex+text.length))).insertBefore(textNode);
    }
    $(textNode).remove();
...
//removes the green selection on highlighted text
//textEl is the highlighted text element(span)
//should return the new textNode and startIndex of the text that was linked
amzn_ps_bm_link.prototype.removeHighlightedText = function(textEl) {
    try {
        var self=this;
        var $=amznpubstudio_jQuery;
        ...
        if(textEl.length>0) {
            var text = "";
            var prevSib = $(textEl.get(0).previousSibling);
            if( prevSib.length>0 && prevSib.get(0).nodeType == 3) {
                text += prevSib.text();
                prevSib.remove();
            }
            ...
            text += textEl.text();
            var nextSib = $(textEl.get(0).nextSibling);
            if( nextSib.length>0 && nextSib.get(0).nodeType == 3) {
                text += nextSib.text();
                nextSib.remove();
            }
            var textNode = $(DOCUMENT.createTextNode(text)).insertBefore(textEl);
            textEl.remove();

            return {'textNode':textNode, 'startIndex':startIndex, 'text':textEl.text()};
        }
        else {
            return null;
        }
        ...
        window.amznPs.logger.log("Error in removeHighlightedText - " + e.message);
    }
};


amzn_ps_bm_link.prototype.attachTextLinkEventToCompose = function(cb) {
    var self = window.amznPs.link;
    var $ = amznpubstudio_jQuery;
    var composeBody = window.amznPs.bootStrap.composeBody;
    if(composeBody && composeBody.length>0) {
        var marker = composeBody.data('amznPsBmMarker');
        if(!marker) {
            window.amznPs.logger.log('marking compose-body and attaching keyup for link-creation');
            composeBody.data('amznPsBmMarker', 'marked');
            composeBody.unbind('click', self.allowComposeModeLinkCreation);
            composeBody.bind('click', {globalScope:self}, self.allowComposeModeLinkCreation);
            composeBody.unbind('keyup', self.keyboardSelectionWatcher);
            composeBody.bind('keyup', {globalScope:self,mode:'compose',getSelection:self.getSelectionInComposeMode}, self.keyboardSelectionWatcher);
        }
    }
    if(cb) cb();
};
amzn_ps_bm_link.prototype.deAttachTextLinkEventToCompose = function(cb) {
    var self = window.amznPs.link;
    var $ = amznpubstudio_jQuery;
    var composeBody = window.amznPs.bootStrap.composeBody;
    if(composeBody && composeBody.length>0) {
    ...
    window.amznPs.link.initLinking();
    if(cb) cb();
};
amzn_ps_bm_link.prototype.keyboardSelectionWatcher = function(event) {
    var self =  event.data.globalScope;
    var mode =  event.data.mode;
    var getSelection = event.data.getSelection;
    var curSelection = getSelection();

    if ( event && event.keyCode === 27 ) { // for escape key
        linkingUtils.removeEditDeleteBox();
        linkingUtils.removeCreateLinkBox();
        return;
    }

    var cb =  function(event) {
        var self = event.data.globalScope, mode = event.data.mode;
        if(mode === 'compose') self.allowComposeModeLinkCreation( event);
        if(mode === 'html') self.allowHtmlModeLinkCreation(event);
    };

    linkingUtils.selectionWatcher(getSelection, getSelection(), cb, event);
};

amzn_ps_bm_link.prototype.attachTextLinkEventToHtml = function(cb) {
    var self = window.amznPs.link;
    var $ = amznpubstudio_jQuery;
    var htmlTextArea = window.amznPs.bootStrap.htmlTextArea;
    if(htmlTextArea && htmlTextArea.length>0) {
        var marker = htmlTextArea.data('amznPsBmMarker');
        if(!marker) {
            htmlTextArea.unbind('click', self.allowHtmlModeLinkCreation);
            window.amznPs.logger.log('marking html-textArea and attaching mouseup for link-creation');
            htmlTextArea.data('amznPsBmMarker', 'marked');
            htmlTextArea.bind('click', {globalScope:self}, self.allowHtmlModeLinkCreation);
            htmlTextArea.unbind('keyup', self.keyboardSelectionWatcher);
            htmlTextArea.bind('keyup', {globalScope:self,mode:'html',getSelection:self.getSelectionInHtmlMode}, self.keyboardSelectionWatcher);
        }
    }
    if(cb) cb();
};

amzn_ps_bm_link.prototype.deAttachTextLinkEventToHtml = function(cb) {
    var self = window.amznPs.link;
    var $ = amznpubstudio_jQuery;
    var htmlTextArea = window.amznPs.bootStrap.htmlTextArea;
    if(htmlTextArea && htmlTextArea.length>0) {
...
amzn_ps_bm_link.prototype.attachTextLinkEvents = function() {
    var self = this;
    var $ = amznpubstudio_jQuery;

    self.attachTextLinkEventToCompose();
    self.attachTextLinkEventToHtml();
};

amzn_ps_bm_link.prototype.allowHtmlModeLinkCreation = function(event) {
    var $ = amznpubstudio_jQuery;
    var self =  event.data.globalScope;
    if(event.target.tagName.toLowerCase()== "img") return;

    clearTimeout(self.textSelectionTimer);
    self.textSelectionTimer =setTimeout((function() {
        return(function() {
            searchbox.closeSearchBox();
            self.initiateAdminOpsForTextInHtml(event);
        });
    })(), 500);
...
amzn_ps_bm_link.prototype.allowComposeModeLinkCreation = function(event) {
    var $ = amznpubstudio_jQuery;
    var self =  event.data.globalScope;
    if(event.target.tagName.toLowerCase()== "img") {
        event.data.globalScope = self;
        self.attachClickToImage(event);
        return;
    }
    ...
    self.textSelectionTimer =setTimeout((function() {
        return (function() {
            self.initiateAdminOpsForTextInCompose(event.type);
        });
    })(), 500);
};

//el is the element which should be visibile, iframe may/may-not be there
amzn_ps_bm_link.prototype.isPresent = function(el, iframe) {
    var self = this;
    var $ = amznpubstudio_jQuery;

    var checkVisibilityAttrs = function(el) {
        if(el) {
            return el.css('opacity')>0 && el.css('display')!='none' && el.css('visibility')!='hidden';
        }
        else {
            return false;
        }
    ...
    var checkVisForParents = function(el) {
        var vis = true;
        if(el) {
            el.parents().each(function() {
                if(!checkVisibilityAttrs($(this))) {
                    vis = false;
                    return false;   //needed to break out of the loop
                }
            });
        }
        else {
    ...
    return checkVisibilityAttrs(el) && (!iframe || checkVisibilityAttrs(iframe)) &&
        checkVisForParents(el) && (!iframe || checkVisForParents(iframe));
};

amzn_ps_bm_link.prototype.getAnchorParent = function(curPar) {
    var self = this;
    var $=amznpubstudio_jQuery;
...
amzn_ps_bm_link.prototype.returnModeType = function() {
    var self=this;
    var $=amznpubstudio_jQuery;

    var bootStrap = window.amznPs.bootStrap;

    var isComposeMode, isHtmlMode;
    isComposeMode = self.isPresent(bootStrap.composeBody, bootStrap.composeIframe);
    isHtmlMode = self.isPresent(bootStrap.htmlTextArea, bootStrap.htmlIframe);

    if(!isComposeMode && isHtmlMode) {
        window.amznPs.logger.log('html mode is on');
        return 'html';
    }
    else if(isComposeMode && !isHtmlMode) {
        window.amznPs.logger.log('compose mode is on');
        return 'compose';
    }
    else {
        window.amznPs.logger.log('cannot decide which mode is on');
        return null;
    }
};

amzn_ps_bm_link.prototype.chooseModeAndInitiateAdminOps = function() {
    var self=this;
    var $=amznpubstudio_jQuery;
    var searchBoxLeft =parseInt(window.amznPs.bootStrap.getData('amzn_search_box_left') || 10);
    var searchBoxTop = parseInt(window.amznPs.bootStrap.getData('amzn_search_box_top') || 10);
    var searchBoxPosition = {'left':searchBoxLeft,'top':searchBoxTop,'cookie':true};
    var modeType = self.returnModeType();

    if(!modeType || modeType.length==0) {
        window.amznPs.logger.log('do not open the search-box because the mode-type is not clear');
    }
    else if(modeType=='html') {
        self.initiateAdminOpsForTextInHtml();
    }
    else if(modeType=='compose') {
        self.initiateAdminOpsForTextInCompose();
    }
};

amzn_ps_bm_link.prototype.initiateAdminOpsForTextInHtml = function(event) {
    ...
    var $=amznpubstudio_jQuery;
    var bootStrap = window.amznPs.bootStrap;
    var textArea = bootStrap.htmlTextArea, curSelection = null;
    var selRestoreObj;
    if(textArea && textArea.length>0) {
        var value = textArea.val();
        var startPos = textArea.get(0).selectionStart;
        var endPos = textArea.get(0).selectionEnd;
        curSelection = value.substring(startPos, endPos);
        selRestoreObj= {selTarget:textArea.get(0),selectionStart:startPos,selectionEnd:endPos};
    }
    else {
        curSelection = bootStrap.htmlWindow.getSelection();
    }

    var text = curSelection+'';
    if(text.length>1 ) {
        searchbox.updateEditHotspot('text', {selectionStart:selRestoreObj.selectionStart,selectionEnd:selRestoreObj.selectionEnd}, 0, text, 'html');
        var searchObj = {callback:function(){
            selRestoreObj.selTarget.selectionStart  = selRestoreObj.selectionStart;
            selRestoreObj.selTarget.selectionEnd  = selRestoreObj.selectionEnd;
        }, searchTerm:text,linkType : 'textLink'};
        if(!event) {
            var searchNode = {'left':30,'top':30,'cookie':true,linkType:'textLink'};
            var searchObj = {callback:function(){
                selRestoreObj.selTarget.selectionStart  = selRestoreObj.selectionStart;
                selRestoreObj.selTarget.selectionEnd  = selRestoreObj.selectionEnd;
            }, searchTerm:text,node : searchNode};
            searchbox.buildSearchBox(searchObj);
            return true;
        }
        if(event.type === 'click') {
            var selObj = {left:event ? (event.clientX-5) || $(DOCUMENT).width()-170 :$(DOCUMENT).width()-170 ,
                ...
                'searchTerm':text/*,mode:'html'*/,
                'ifr': window.amznPs.bootStrap.htmlIframe,
                'win': window.amznPs.bootStrap.htmlWindow};
            linkingUtils.openCreateLinkBox(selObj, window.amznPs.bootStrap.tag);
            return true;
        }
    }
    else {
        searchbox.editHotSpot.type = 'text';
        searchbox.editHotSpot.mode = 'html';
        return false;
    }
};

//validates if highlighted text is correct and opens search-box
amzn_ps_bm_link.prototype.initiateAdminOpsForTextInCompose = function(eventType) {
    if(!bootstrap.isLinkingEnabled()) {
        return;
    }
    ...
    searchbox.closeSearchBox();
    try {
        var curSelection = window.amznPs.bootStrap.composeWindow.getSelection();
        var _searchTerm = curSelection.toString();
        searchbox.editHotSpot.el = $(curSelection.focusNode);
        searchbox.editHotSpot.type = 'none';
        searchbox.editHotSpot.mode = 'compose';

        //moved from findValidTextNode, might be used for restoring cursor position
        if(curSelection && curSelection.anchorNode && curSelection.focusNode) {
            searchbox.editHotSpot.el = $(curSelection.focusNode);
            searchbox.editHotSpot.type = 'none';
            searchbox.editHotSpot.mode = 'compose';
        ...
        var textMap = linkingUtils.findValidTextNode(curSelection, true);
        //var savedSel = utils.saveSelection(window.amznPs.bootStrap.composeBody.get(0),window.amznPs.bootStrap.composeWindow);
        var textObject = linkingUtils.getTextPosition(textMap.text || '', textMap.textNode, textMap.localStart,window.amznPs.bootStrap.composeWindow);
        textMap.textNode = textObject.textNode;
        //utils.restoreSelection(window.amznPs.bootStrap.composeBody.get(0),savedSel,window.amznPs.bootStrap.composeWindow);
        if(textMap){
            searchbox.updateEditHotspot('text',
                {'textNode':textMap.textNode,'startIndex':textMap.localStart}, 0, textMap.text, 'compose');
            var _anchor =self.getAnchorParent(textMap.textNode.parent());
            if(_anchor != null ) {
                var keyword = _anchor.attr('data-amzn-ps-bm-keyword');
                if(!keyword || keyword.length==0) {
                    keyword = _anchor.attr('data-blogger-escaped-data-amzn-ps-bm-keyword')
                }
                if(_anchor.hasClass(self.linkTypeToCssClassMap['textLink'])) {
                    linkingUtils.openEditDeleteBox({'link': _anchor, 'type': 'textLink', 'left': textObject.left + 5,
                            'top': textObject.top + 5, 'searchTerm': keyword, 'body':window.amznPs.bootStrap.composeBody,
                            'win': window.amznPs.bootStrap.composeWindow, 'ifr': window.amznPs.bootStrap.composeIframe},
                        window.amznPs.bootStrap.tag, self.deleteLink);
                }

                return true;
            }
            if(_searchTerm.length>1) {
                textObject.linkType = 'textLink';
                textObject.searchTerm = _searchTerm;
                textObject.ifr = window.amznPs.bootStrap.composeIframe;
                textObject.win = window.amznPs.bootStrap.composeWindow;

                var _anchor =self.getAnchorParent(textMap.textNode.parent());
                linkingUtils.openCreateLinkBox(textObject, window.amznPs.bootStrap.tag);
                return true;
            }
            return false;
        }
    } catch (e) {
        window.amznPs.logger.log("Error in initiateAdminOpsForTextInCompose - " + e.message);
    }
};

amzn_ps_bm_link.prototype.changeLinkProtocol = function(link , protocol) {
    var $ = amznpubstudio_jQuery;
    var tempLink = $('<a>').attr('href', link).get(0);
    tempLink.protocol=protocol;
    var href = tempLink.href;
    return href;
};

amzn_ps_bm_link.prototype.baseUrlFormatter = {
    'SEARCH': function(curResult, refTag) {
        ...
        var currentCategory = searchbox.getSearchAliasFromCategory(selected.val());

        return config.retailWebsite + "/s/ref=" + refTag + "/?" +
            "url=search-alias%3D" + currentCategory + "&field-keywords=" + currentSearchTerm + "&";
    },
    'CATEGORY': function(curResult, refTag) {
        return  config.retailWebsite + "/b/ref=" + refTag + "/?" +
            'node=' + curResult.category.browseNode + '&';
    },
    'GATEWAY': function(curResult, refTag){
        return config.retailWebsite + "/ref=" + refTag + "/?";
    },
    'ASIN': function(curResult, refTag){
        return curResult.DetailPageURL + "/ref=" + refTag + "/?";
    }
};

amzn_ps_bm_link.prototype.getBaseUrlForLink = function(curResult, type, refTag) {
  return this.baseUrlFormatter[type].call(this, curResult, refTag);
}

amzn_ps_bm_link.prototype.wrapTextInLink = function(curResult, editData, type) {
    var self = this;
    var $ = amznpubstudio_jQuery;
    var _bootStrap = window.amznPs.bootStrap;
    var linkcode = window.amznPs.linkCodesConfigData.textLink;

    var previousLinkId = 'na';
    var isUpdated =0;
    if( editData && (editData.link) && (editData.link.attr('data-amzn-link-id'))) {
        previousLinkId = editData.link.attr('data-amzn-link-id');
        isUpdated = 1;
    }
    var linkIdData = {
        tag: _bootStrap.tag,
        marketplaceId: config.marketplaceConfig[curResult.Marketplace].id,
        linkCode: linkcode,
    ...
    var linkId = AmznLinkID.get(linkIdData, config.stage);

    var baseUrl = self.getBaseUrlForLink(curResult, type, window.amznPs.refTagsConfigData.textLink);
    var landingPageUrl = baseUrl + "tag=" + _bootStrap.tag + "&linkId=" + linkId + "&linkCode=" + linkcode;

    var href = self.changeLinkProtocol(landingPageUrl,'http:');

    var id = Math.ceil(Math.random()*10000000);
    var tLStr = '<a rel="nofollow" target="_blank" class="amzn_ps_bm_tl" ' +
        'data-amzn-ps-bm-keyword="' + curResult.editHotSpot.searchTerm + // using HTML5 compliant attribute storage (prefixed data-)
        '"data-amzn-link-id="'+linkId+
        '" amzn-ps-bm-asin="' + curResult.ASIN +
        '" id="amznPsBmLink_' + id +
        '" href="' + href + '">' + '__TEXT__CONTENT__' + '</a>';

    var action = 'create';
    var text='', highlightedText = curResult.editHotSpot.text;
    //use the hightlighted text in case it is present
    if(editData && editData.type=='textLink') {
        text=editData.link.text();
        tLStr = tLStr.replace('__TEXT__CONTENT__', text);
    }
    else if(curResult.editHotSpot.type =='text' && highlightedText && highlightedText.length>0) {
        text=highlightedText;
        tLStr = tLStr.replace('__TEXT__CONTENT__', highlightedText);
    }
    //use the search-term in case no text is highlighted
    else {
        text=curResult.editHotSpot.searchTerm;
        tLStr = tLStr.replace('__TEXT__CONTENT__', text);
    }

    var imgPixel = window.amznPs.metrics.createTLPixel(id, linkId);

    var curModeType = self.returnModeType();
    var linkModeType = curResult.editHotSpot.mode;

    var $S = window.amznPs.strings;

    //this should not happen because on-change of the mode, search-box is closed
    if(linkModeType && curModeType != linkModeType) {
        window.amznPs.notifications.showWarningNotification($S.get('ps-bk-text-link-fail-notification'));
        window.amznPs.logger.log('link mode-type and current mode-type do not match, do no do anything');
    }
    else {
        if(curModeType=='html') {
            var textArea = window.amznPs.bootStrap.htmlTextArea;
            var value = textArea.val();
            var selPosObj = curResult.editHotSpot.el || { selectionStart:textArea.get(0).selectionStart, selectionEnd:textArea.get(0).selectionEnd};
            var startPos = selPosObj.selectionStart;
            var endPos = selPosObj.selectionEnd;
            textArea.val(value.substring(0,startPos)+'\n'+tLStr+imgPixel+value.substring(endPos));
            window.amznPs.notifications.showSuccessNotification($S.get('ps-bk-text-link-success-notification'));
            var _payLoad = {ASIN:curResult.ASIN,mode:'html',action:'create',data:self.text ,type: window.amznPs.linkCodesConfigData.textLink,linkId:linkId,isUpdated:isUpdated,previousLinkId:previousLinkId};
            window.amznPs.metrics.send(_payLoad);
        }
        else {
            var successNotificationMsg = '';
            var textLink = $(tLStr);
            if(!editData) {
                var selectionType = curResult.editHotSpot.type;
                if(selectionType=='text') {
                    var selectedTextEl=curResult.editHotSpot.el;
                    var textNode = selectedTextEl.textNode;

                    //show error message if the text is already linked
                    if(linkingUtils.checkIfAnchorParent(textNode.parent())) {
                        window.amznPs.notifications.showWarningNotification($S.get('ps-link-textlink-err'));
                        return;
                    }
                    else {
                        //if text is highlighted, link it
                        //else insert the search-keyword-text in the page and link it
                        self.insertLinkInTextNode(highlightedText,textNode,selectedTextEl.startIndex,textLink);
                    }
                }
                //find the cursor position and insert the search-keyword-text in the page and link it
                else if(selectionType=='image') {
                    var image = curResult.editHotSpot.el;
                    textLink.insertAfter(image);
                }
                else if(selectionType=='none') {
                    var focusNode = curResult.editHotSpot.el;
                    //if focus is a text-node, add link after the parent of the text-node, o/w the node will break in between
                    if(focusNode.get(0).nodeType==3) {
                        focusNode=focusNode.parent();
                    }
                    if(focusNode && focusNode.get(0).nodeName.toLowerCase()!='body') {
                        textLink.insertAfter(focusNode);
                    }
                    else {
                        textLink.appendTo(window.amznPs.bootStrap.composeBody);
                    }
                }
                else {
                    textLink.appendTo(window.amznPs.bootStrap.composeBody);
                }
                successNotificationMsg = $S.get('ps-bk-text-link-success-notification');
            }
            else if(editData && editData.type=='textLink') {
                var textData = self.deleteLink(editData.link, editData.type,true,window.amznPs.bootStrap.composeBody);
                self.insertLinkInTextNode(textData.text,textData.textNode,textData.startIndex,textLink);
                successNotificationMsg = $S.get('ps-bk-text-link-edit-success-notification');
                action = 'edit';
            }
            else {
                //insert text-link with search-keyword after the selected element
                textLink.insertAfter(editData.link);
                successNotificationMsg = $S.get('ps-bk-text-link-success-notification');
            }

            $(imgPixel).insertAfter(textLink);
            window.amznPs.notifications.showSuccessNotification(successNotificationMsg);
            var _payLoad = {ASIN:curResult.ASIN,data:text,type: window.amznPs.linkCodesConfigData.textLink,mode:'compose',action:action,linkId:linkId,isUpdated:isUpdated,previousLinkId:previousLinkId};
            window.amznPs.metrics.send(_payLoad);
        }
    }
};

amzn_ps_bm_link.prototype.getImageSource = function(img) {
    var self = this;
    var $ = amznpubstudio_jQuery;

    var src = img.attr("src");
    return (src && src.length>0) ? src : img.css('background-image');
};

amzn_ps_bm_link.prototype.wrapImageInLink = function(curResult, editData, type) {
    var self = this;
    var $ = amznpubstudio_jQuery;

    var $S = window.amznPs.strings;

    var parseImageSource = function(str) {
        var img = $(str);
        if(img && img.length>0) {
            return self.getImageSource(img);
        }
        else {
            return str;
        }
    ...
    var _bootStrap = window.amznPs.bootStrap;
    var linkcode = window.amznPs.linkCodesConfigData.imageLink;
    var previousLinkId = 'na';
    var isUpdated =0;
    if( editData && (editData.link) && (editData.link.attr('data-amzn-link-id'))) {
        previousLinkId = editData.link.attr('data-amzn-link-id');
    ...
    var baseUrl = self.getBaseUrlForLink(curResult, type, window.amznPs.refTagsConfigData.imageLink);
    var action = 'create';
    var landingPageUrl = baseUrl + "tag=" + _bootStrap.tag + "&linkId=" + linkId + "&linkCode=" + linkcode;
    ...
    var iLStr = '<a rel="nofollow" target="_blank" class="amzn_ps_bm_il" ' +
        'data-amzn-ps-bm-keyword="' + curResult.editHotSpot.searchTerm +
        '"data-amzn-link-id="'+linkId+
        '" amzn-ps-bm-asin="' + curResult.ASIN +
        '" id="amznPsBmLink_' + id +
        '" href="'+ href + '">' + '__TEXT__CONTENT__' +  '</a>';

    var imgPixel = window.amznPs.metrics.createILPixel(id, linkId);

    var curModeType = self.returnModeType();
    var linkModeType = curResult.editHotSpot.mode;

    if(curModeType != linkModeType) {
        window.amznPs.notifications.showWarningNotification($S.get('ps-bk-image-link-fail-notification'));
        window.amznPs.logger.log('link mode-type and current mode-type do not match, do not create link');
    }
    else {
        if(curModeType=='html') {
            var textArea = window.amznPs.bootStrap.htmlTextArea;
            ...
            if(text && text.length>0) {
                var imgSrc = parseImageSource(text);
                iLStr=iLStr.replace('__TEXT__CONTENT__', text);
                textArea.val(value.substring(0,startPos)+'\n'+iLStr+imgPixel+value.substring(endPos));
                window.amznPs.notifications.showSuccessNotification($S.get('ps-bk-image-link-success-notification'));
                var _payLoad = {ASIN:curResult.ASIN,mode:'html',action:'create',data:imgSrc,type: window.amznPs.linkCodesConfigData.imageLink,linkId:linkId,isUpdated:isUpdated,previousLinkId:previousLinkId};
                window.amznPs.metrics.send(_payLoad);
            }
            else {
                window.amznPs.notifications.showWarningNotification($S.get('ps-bk-image-link-fail-notification'));
                window.amznPs.logger.log('image is not selected, do not create link');
                return;
            }
        }
        else {
            if(editData && editData.type!='imageLink' && editData.type!='imageOnlyProductLink') {
                window.amznPs.notifications.showWarningNotification($S.get('ps-bk-image-link-fail-notification'));
                window.amznPs.logger.log('Could not find any selected image');
                return;
            }
            else {
                var selectedImageEl, successNotificationMsg;

                if(!editData) {
                    if(curResult.editHotSpot.type=='image') {
                        selectedImageEl=curResult.editHotSpot.el;
                        if(linkingUtils.checkIfAnchorParent(selectedImageEl)) {
                            window.amznPs.bootStrap.composeWindow.getSelection().removeAllRanges();
                            window.amznPs.notifications.showWarningNotification($S.get('ps-bk-image-already-linked-notification'));
                            window.amznPs.logger.log('The image is already linked');
                            return;
                        }
                        successNotificationMsg = 'Image tagged successfully';
                    }
                    else {
                        window.amznPs.bootStrap.composeWindow.getSelection().removeAllRanges();
                        window.amznPs.notifications.showWarningNotification($S.get('ps-bk-image-link-fail-notification'));
                ...
                else if (editData && editData.type=='imageLink') {
                    var el = editData.link;
                    selectedImageEl=el.children('img');
                    //remove the old imageLink and add the new one
                    self.deleteLink(el, 'imageLink',true,window.amznPs.bootStrap.composeBody);
                    successNotificationMsg = $S.get('ps-bk-image-link-edit-success-notification');
                    action= 'edit';
                }
                else if (editData && editData.type=='imageOnlyProductLink') {
                    //remove the old imageLink and add the new one
                    var el = editData.link;
                    selectedImageEl=el.children('img.amzn_ps_bm_ipl_image');
                    selectedImageEl.removeClass('amzn_ps_bm_ipl_image');
                    selectedImageEl.insertBefore(el);
                    self.deleteLink(el, 'imageOnlyProductLink',true,window.amznPs.bootStrap.composeBody);
                    successNotificationMsg= $S.get('ps-bk-image-link-edit-success-notification');
                    action= 'edit';
                }

                var imgSrc = self.getImageSource(selectedImageEl);

                iLStr=iLStr.replace('__TEXT__CONTENT__', '');
                var iL = $(iLStr);
                iL.insertBefore(selectedImageEl);
                var iP = $(imgPixel);
                iP.insertBefore(selectedImageEl);
                selectedImageEl.appendTo(iL);
                window.amznPs.notifications.showSuccessNotification(successNotificationMsg);
                var _payLoad = {ASIN:curResult.ASIN,mode:'compose',action:action,data:imgSrc,type: window.amznPs.linkCodesConfigData.imageLink,linkId:linkId,isUpdated:isUpdated,previousLinkId:previousLinkId};
                window.amznPs.metrics.send(_payLoad);
                window.amznPs.link.attachEditDeleteEvent(iL, 'click', 'imageLink', 'none');
            }
        }
    }
};

amzn_ps_bm_link.prototype.getSearchTerm = function($img) {
    var $imgParent = $img.parent();
    var keyword = $imgParent.attr('data-amzn-ps-bm-keyword');
    if(!keyword || keyword.length==0) {
        keyword = $imgParent.attr('data-blogger-escaped-data-amzn-ps-bm-keyword');
    }
    var _alt = $img.attr('alt');
    var _searchTerm = keyword || _alt;
    return _searchTerm|| $img.attr('title');
};

amzn_ps_bm_link.prototype.attachClickToImage = function(event) {
    var self = event.data.globalScope || this;
    var $ = amznpubstudio_jQuery;
    var img = $(event.target);
    clearTimeout(self.imageSelectionTimer);
    self.imageSelectionTimer = setTimeout((function(img, ev) {
        return (function() {
            var left = ev.pageX, top = ev.pageY;
            var imgParent = img.parent();
            var keyword = self.getSearchTerm(img);
            var _linkType ='imageLink';
            var _hasClass = false;
            for (var _link in self.linkTypeToCssClassMap)
                if (self.linkTypeToCssClassMap.hasOwnProperty(_link)) {
                    var _class = self.linkTypeToCssClassMap[_link];
                    if(imgParent.hasClass(_class)) { _linkType = _link; /*_hasClass = true;*/break;}
                }

            if(imgParent.is('a')/* && _hasClass*/) {
                searchbox.updateEditHotspot('image', img, 0, keyword, 'compose');
                linkingUtils.openEditDeleteBox({'link':imgParent, 'type':'imageLink', 'left':left,
                    'top':top, 'searchTerm':keyword || '', 'body':window.amznPs.bootStrap.composeBody,
                    'win':window.amznPs.bootStrap.composeWindow, 'ifr':window.amznPs.bootStrap.composeIframe},
                    window.amznPs.bootStrap.tag, self.deleteLink);
            }
            //even though the image is not linked using bookmarklet, it might have a link created by the publisher
            //check with PM in case we need to edit that as well. the link could be at upper levels as well
            else {
                searchbox.updateEditHotspot('image', img, 0, null, 'compose');
                linkingUtils.openCreateLinkBox({"left":left,"top":top,"width":0,"height":0,linkType : _linkType,
                    'searchTerm':keyword || '',
                    'ifr': window.amznPs.bootStrap.composeIframe,
                    'win' : window.amznPs.bootStrap.composeWindow}, window.amznPs.bootStrap.tag);
            }
        });
    })(img, event), 500);
};

amzn_ps_bm_link.prototype.initPageMonitors = function() {
    var self = this;
    var $ = amznpubstudio_jQuery;
    $(DOCUMENT).unbind('click', self.attachImageLinkEvents);
    $(DOCUMENT).unbind('click', self.attachEditDeleteEventsToLinks);
    $(DOCUMENT).bind('click', {globalScope:self}, self.attachImageLinkEvents);
    $(DOCUMENT).bind('click', {globalScope:self}, self.attachEditDeleteEventsToLinks);
};

amzn_ps_bm_link.prototype.attachImageLinkEvents = function(event) {
    var $ = amznpubstudio_jQuery;
    var self;
    if(event) {
        self = event.data.globalScope;
    }
    else {
        self = this;
    }

    clearTimeout(self.imageClickInComposeTimer);
    self.imageClickInComposeTimer = setTimeout((function() {
        return (function() {
            window.amznPs.logger.log('attaching click-listener to images');

            var attachClickToImages = function(imgList) {
                var batchSize = 50;
                for(var i=0;i<imgList.length && i<50;i++) {
                    var img = $(imgList[i]);
                    var marker = img.data('amznPsBmMarker');
                    if(!marker) {
                        window.amznPs.logger.log('marking image - ' + img + ' and attaching click listener for linking');
                        img.data('amznPsBmMarker','marked');
                        img.unbind('click', self.attachClickToImage);
                        img.bind('click', {globalScope:self}, self.attachClickToImage);
                       if($.browser.msie) { // incase of ie attaching mouseup, to take care of  down event bubbling
                            img.unbind('mouseup', self.attachClickToImage);
                            img.bind('mouseup', {globalScope:self}, self.attachClickToImage);
                        }
                    }
                }
                imgList.splice(0, i);
                ...
                if(imgList.length>0) {
                    setTimeout((function(imgList) {
                        return (function() {
                            attachClickToImages(imgList);
                        })
                    })(imgList), 200)
                }
            };
            ...
            var composeBody = window.amznPs.bootStrap.composeBody;
            if(composeBody && composeBody.length>0) {
                //do not run for the images which are created using image-only-product-link feature of bookmarklet
                attachClickToImages(composeBody.find('img:not(.amzn_ps_bm_ipl_image)'));
            }
        });
    })(), 500);
};

amzn_ps_bm_link.prototype.removePixelId = function (linkId, $composeBody) {

    var i;
    var pixelId = '#amznPsBmPixel_' + linkId.split('_')[1];
    var pixelEl = ($composeBody).find(pixelId);
    var $composeBodyImg = $composeBody.find('img');
    if (pixelEl) {
        pixelEl.remove();
    }
    if ($composeBodyImg) {
        if ($composeBody.find('img')) {
            console.log($composeBody.find('img'));
            for (i = 0; i < $composeBodyImg.length; i++) {
                console.log($composeBodyImg[i].id);
                if ($composeBodyImg[i].id) {
                    if (('#' + $composeBodyImg[i].id) == pixelId) {
                        $composeBodyImg[i].remove();
                    }
                }
            }
        }
        ...
        console.log("No pixels to delete");
};

amzn_ps_bm_link.prototype.deleteLink = function(link, type,hideNotification,$composeBody) {
    var self = window.amznPs.link;
    var $ = amznpubstudio_jQuery;
    var $S = window.amznPs.strings;

    var notificationMsg = "";
    var linkId = link.attr('id'), retData=null;
    // .find was not working in safari so using custom function
    if (linkId) {
        // .find not working in safari so custom remove function .
        self.removePixelId(linkId, $composeBody);
    }
    var asin = link.attr('amzn-ps-bm-asin');
    if(!asin || asin.length==0) {
        asin=link.attr('data-blogger-escaped-amzn-ps-bm-asin');
    }
    var _linkID = link.attr('data-amzn-link-id');
    if(type=='imageLink') {
        var img =link.children('img');
        img.insertAfter(link);
        link.remove();
        if(!hideNotification) {
            var _payLoad = {ASIN:asin,mode:'compose',action:'delete',data:self.getImageSource(img) ,type: window.amznPs.linkCodesConfigData.imageLink,linkId:_linkID,isUpdated:0,previousLinkId:'na'};
            window.amznPs.metrics.send(_payLoad);
        }
        notificationMsg = $S.get('ps-bk-image-link-del-success-notification');
        retData = img;
    }
    else if(type=='textLink') {
        //should return text, textNode, and startIndex
        var text = link.text();
        var textData = self.removeHighlightedText(link);
        if(!hideNotification) {
            var _payLoad = {ASIN:asin,data:text,type: window.amznPs.linkCodesConfigData.textLink,mode:'compose',action:'delete',linkId:_linkID,isUpdated:0,previousLinkId:'na'};
            window.amznPs.metrics.send(_payLoad);
        }
        notificationMsg = $S.get('ps-bk-text-link-del-success-notification');
        retData = textData;
    }
    else if(type=='productLink') {
        link.remove();
        if(!hideNotification) {
            var _payLoad = {ASIN:asin,mode:'compose',action:'delete',type: window.amznPs.linkCodesConfigData.productLink,linkId:_linkID,data:'na',isUpdated:0,previousLinkId:'na'};
            window.amznPs.metrics.send(_payLoad);
        }
        notificationMsg = $S.get('ps-bk-product-link-del-success-notification');
        retData = link;
    }
    else if(type=='imageOnlyProductLink') {
        link.remove();
        if(!hideNotification) {
            var _payLoad = {ASIN:asin,mode:'compose',action:'delete',type: window.amznPs.linkCodesConfigData.imageOnlyProductLink,linkId:_linkID,data:'na',isUpdated:0,previousLinkId:'na'};
            window.amznPs.metrics.send(_payLoad);
        }
        notificationMsg = $S.get('ps-bk-image-only-link-del-success-notification');
        retData = link;
    }

    linkingUtils.removeEditDeleteBox();
    if(notificationMsg !== "" && !hideNotification)
        window.amznPs.notifications.showSuccessNotification(notificationMsg);
    return retData;
};

amzn_ps_bm_link.prototype.insertLinkInTextNode = function(text, textNode, startIndex,$ele) {
	var self=this;
    var $=amznpubstudio_jQuery;
    var content = textNode.text();
    if(startIndex>0) {
...
amzn_ps_bm_link.prototype.attachEditDeleteEvent = function(link, eventType, linkType, selectionType) {
    var self = this;
    var $ = amznpubstudio_jQuery;

    var marker = link.data('amznPsBmMarker');
    if(!marker) {
        link.data('amznPsBmMarker','marked');
        link.unbind(eventType, self.editDeleteEventListener);
        link.bind(eventType, {'self':self, 'selectionType':selectionType, 'linkType':linkType}, self.editDeleteEventListener);
    }
    else {
        window.amznPs.logger.log('edit-delete event is already attached to the element');
    }
};

amzn_ps_bm_link.prototype.editDeleteEventListener = function(e) {
    var self = e.data.self;
    var selectionType = e.data.selectionType;
    var linkType= e.data.linkType;

    var $ = amznpubstudio_jQuery;

    var el = $(this);
    var keyword = el.attr('data-amzn-ps-bm-keyword');
    if(!keyword || keyword.length==0) {
        keyword = el.attr('data-blogger-escaped-data-amzn-ps-bm-keyword');
    }
    searchbox.updateEditHotspot(selectionType, el, 0, keyword, 'compose');
    var _left = e.pageX;
    var _top = e.pageY;
    if(linkType == "productLink") {// incase of iframe, do not render button's on it
        _left = el.position().left - self.editDeleteBoxWidth + 23;
        if(_left < 0) _left  = el.position().left + el.width() +23;
    }

    linkingUtils.openEditDeleteBox({'link':el, 'type':linkType, 'left':_left, 'top':_top, 'searchTerm':keyword,
            'body':window.amznPs.bootStrap.composeBody,
        'win':window.amznPs.bootStrap.composeWindow, 'ifr':window.amznPs.bootStrap.composeIframe},
        window.amznPs.bootStrap.tag, self.deleteLink);
};

amzn_ps_bm_link.prototype.attachEditDeleteEventsToLinks = function(event) {
    var $ = amznpubstudio_jQuery;
    ...
    if(composeBody && composeBody.length>0) {
        //imageLinks already have a click handler for opening create-link button
        //attach edit-delete code there itself
       // var textLinks = composeBody.find('.amzn_ps_bm_tl');
        //var imageOnlyProductLinks = composeBody.find('.amzn_ps_bm_ipl');
        var productLinks = composeBody.find('.amzn_ps_bm_pl');

        var bindEvents = function(eventType, links, linkType, selectionType) {
            var batchSize = 50;
            for(var i=0;i<links.length && i<batchSize;i++) {
                var link = $(links[i]);
                self.attachEditDeleteEvent(link, eventType, linkType, selectionType);
            }
            links.splice(0, i);

            if(links.length>0) {
                setTimeout((function(eventType, links, linkType, selectionType) {
                    return (function() {
                        bindEvents(eventType, links, linkType, selectionType);
                    })
                })(eventType, links, linkType, selectionType), 200)
            }
        };

        bindEvents('mouseenter', productLinks, 'productLink', 'none');
    }
};

amzn_ps_bm_link.prototype.init = function() {
    var self = this;
    var $ = amznpubstudio_jQuery;
    self.attachImageLinkEvents();
    self.attachEditDeleteEventsToLinks();
    self.initPageMonitors();
    self.attachTextLinkEvents();
};
amzn_ps_bm_link.prototype.getSelectionInHtmlMode = function() {
    var self = this;
    var $=amznpubstudio_jQuery;

    var bootStrap = window.amznPs.bootStrap;
    var textArea = bootStrap.htmlTextArea;
...
amzn_ps_bm_link.prototype.getSelectionInComposeMode = function() {
    var curSelection = window.amznPs.bootStrap.composeWindow.getSelection();
    return curSelection;
};

amzn_ps_bm_link.prototype.initLinking = function() {
    var self = window.amznPs.link;
    searchbox.closeSearchBox();
    self.chooseModeAndInitiateAdminOps();
};

function amzn_ps_bm_features() {

}

amzn_ps_bm_features.prototype.chooseActions = function(value, curResult, editData) {
    var self = this;
    var $ = amznpubstudio_jQuery;

    if(value == 'textLink') {
        window.amznPs.link.wrapTextInLink(curResult, editData, window.amznPs.landingPageTypeConfigData.asinPage);
    }
    else if(value == 'imageLink') {
        window.amznPs.link.wrapImageInLink(curResult, editData, window.amznPs.landingPageTypeConfigData.asinPage);
    }
    else if(value == 'copy') {
        self.openCopyToClipBoard(curResult, editData);
    }
    else if(value == 'productLink') {
        self.insertPL(curResult, editData);
    }
    else if(value == 'imageOnlyProductLink') {
        self.insertImageOnlyPL(curResult, editData);
    }

    searchbox.closeSearchBox();
};

amzn_ps_bm_features.prototype.addSearchFeatures = function(curResult, par, i, editData, defaultLinkType) {
    var self = this;
    var $ = amznpubstudio_jQuery;

    var actionsIndicatorWrapper = $('<div/>', {}).addClass('amznps_search_result_action_indicator').appendTo(par);
    var actionsIndicator = $('<div/>', {
        css:{
            "background-image": 'url("' + config.sprite1 + '")'
        }
    }).appendTo(actionsIndicatorWrapper);

    var actionsCont = $('<div/>', {
        id: 'amznActionsCont_'+i,
        click: function(e) {
            e.stopPropagation();
            e.preventDefault();
            return false;
    ...
    }).addClass('amznActionsCont').appendTo(par);
...
*/
    var $S = window.amznPs.strings;
    defaultLinkType = defaultLinkType || 'select';
    var actionList = {
        "select" : {text:$S.get('ps-bk-select-link'),title: $S.get('ps-bk-select-link-title')},
        "textLink":{text: $S.get('ps-bk-text-link'),title: $S.get('ps-bk-text-link-title')},
        "imageLink":{text: $S.get('ps-bk-image-link'),title: $S.get('ps-bk-image-link')}
    };/*
        "productLink":{text: $S.get('ps-bk-product-link'),title: $S.get('ps-bk-product-link-title')},
        "imageOnlyProductLink":{text: $S.get('ps-bk-image-only-link'),title: $S.get('ps-bk-image-only-link-title')},
        "copy":{text: $S.get('ps-bk-copy-to-clipboard'),title: $S.get('ps-bk-copy-to-clipboard-title')}
    };*/
    if( defaultLinkType !== 'select') delete actionList['select'];
    var showDropdown = function($container) {
        $container.addClass('amznps-open-links');
        var $subMenu = $container.children('ul');
        for(var _i=0;_i<$subMenu.length;_i++) {
            var $ele = $($subMenu[_i]);
            if(!$container.hasClass('amznps-open-links')) {
                $container.addClass('amznps-open-links');
            }
            self.scrollIntoView($ele.children().last(),$("div#amznps_search_resultsarea")); // setting focus to last option, because of scroll
        }
        return false;
    };
    var $menu = $('<div/>',{
        id:'amzn-nav-action-'+i,
        css:{
            "background-image": 'url("' + config.sprite1 + '")'
        }
    }).addClass('amzn-nav-action').appendTo(actionsCont);

    var $actionButton = $('<div/>',{
        title: $S.get('ps-bk-select-link-title'),
        mousedown: function(e) {
            if($textHolder.text() === $S.get('ps-bk-select-link')) showDropdown($(this).parent());
            else {
                curResult.editHotSpot = searchbox.editHotSpot;
                self.chooseActions($(this).children('span').attr('actionKey'), curResult, editData);
            }
        }
    }).addClass('amzn-nav-action-text-btn').appendTo($menu);

    var $textHolder = $('<span/>',{
        text: $S.get('ps-bk-select-link'),
        'actionKey':'select'
    }).addClass('amzn-nav-action-text').appendTo($actionButton);

    var $dropButton = $('<div/>',{
        id: 'amzn-nav-action-drop-btn-'+i,
        title: $S.get('ps-bk-select-link-title'),
        mousedown: function(e){
            showDropdown($(this).parent());
        }
       }).addClass('amzn-nav-action-drop-btn').appendTo($menu);

    var $imageHolder = $('<div/>',{
        css: {
            "background-image": 'url("' + config.sprite1 + '")'
        }
    }).addClass('amzn-nav-action-image').appendTo($dropButton);

    var $list = $('<ul/>',{
     }).appendTo($menu);

    $menu.mouseleave(function(){
        $menu.removeClass('amznps-open-links');
    });

    $list.delegate('li','click',(function(curResult, editData) {
            return (function(e) {
                var linkType = $(this).attr('link-type');
                if(linkType === "select") return;
                curResult.editHotSpot = searchbox.editHotSpot;
                $textHolder.text($(this).text());
                $textHolder.attr('actionKey',linkType);
                self.chooseActions(linkType, curResult, editData);
            });
        })(curResult, editData)
    );

    for(var linkType in actionList) {
        $('<li/>',{
            'link-type':linkType,
            text:actionList[linkType].text,
            title:actionList[linkType].title
            }).addClass('dropdown-li').appendTo($list);
    }
    defaultLinkType = editData ? editData.type : defaultLinkType;
    var selectedOpt = actionList[defaultLinkType].text;
    $textHolder.attr('actionKey',defaultLinkType);
    $textHolder.text(selectedOpt);
    $actionButton.attr('title',actionList[defaultLinkType].title);
};

amzn_ps_bm_features.prototype.insertLinkInTextArea = function(linkStr) {
    var self = this;
    var $ = amznpubstudio_jQuery;
    ...
    var endPos = textArea.get(0).selectionEnd;
    if(endPos>0) {
        textArea.val(value.substring(0,endPos)+'\n'+linkStr+value.substring(endPos));
    }
    else {
        //by-default the end-position is 0, so need to insert the link at the end
        textArea.val(value+'\n'+linkStr);
    }
};

amzn_ps_bm_features.prototype.insertLinkInCompose = function(link, linkType, curResult, editData,linkId) {
    var self = this;
    var $ = amznpubstudio_jQuery;
    ...
    var editHotSpot = curResult.editHotSpot;
    if(!editData) {
        if(editHotSpot) {
            if(editHotSpot.type=='text') {
                var textNode = $(editHotSpot.el.textNode);
                var text = editHotSpot.text;
                var startIndex = editHotSpot.el.startIndex+text.length;
                var content = textNode.text();
                if(startIndex>0) {
                    $(DOCUMENT.createTextNode(content.substring(0,startIndex))).insertBefore(textNode);
                }
                link.insertBefore(textNode);
                if(startIndex<content.length) {
                    $(DOCUMENT.createTextNode(content.substring(startIndex))).insertBefore(textNode);
                }
                textNode.remove();
            }
            else if(editHotSpot.type=='image') {
                var image = editHotSpot.el;
                link.insertAfter(image);
            }
            else if(editHotSpot.type=='none'){
                var focusNode = editHotSpot.el;
                //if focus is a text-node, add link after the parent of the text-node, o/w the node will break in between
                if(focusNode.get(0).nodeType==3) {
                    focusNode=focusNode.parent();
                }
                if(focusNode && focusNode.get(0).nodeName.toLowerCase()!='body') {
                    link.insertAfter(focusNode);
                }
                else {
                    link.appendTo(window.amznPs.bootStrap.composeBody);
                }
            }
            else {
                link.appendTo(window.amznPs.bootStrap.composeBody);
        ...
        var editEl = editData.link, editType = editData.type;
        link.insertAfter(editEl);
        if(editType==linkType) {
            //delete the old link
            window.amznPs.link.deleteLink(editEl, linkType,true,window.amznPs.bootStrap.composeBody);
            actionType='edited';
            action ='edit';
        }
        else {
        ...
        //do not delete the old-link in case the option chosen is a different one
        //just creating the new one
    }
    ...
    if(linkType=='productLink') {
        if(actionType=='edited') {
            window.amznPs.notifications.showSuccessNotification($S.get('ps-bk-product-link-edit-success-notification'));
        }
        else {
            window.amznPs.notifications.showSuccessNotification($S.get('ps-bk-product-link-success-notification'));
        }
        eventType='mouseenter';
        window.amznPs.link.attachEditDeleteEvent(link, eventType, linkType, 'none');
        var _payLoad = {ASIN:curResult.ASIN,mode:'compose',action:action,type: window.amznPs.linkCodesConfigData.productLink,linkId:linkId};
        window.amznPs.metrics.send(_payLoad);
    }
    else {
        if(actionType=='edited') {
            window.amznPs.notifications.showSuccessNotification($S.get('ps-bk-image-only-link-edit-success-notification'));
        }
        else {
            window.amznPs.notifications.showSuccessNotification($S.get('ps-bk-image-only-link-success-notification'));
        }
        eventType='click';
        var _payLoad = {ASIN:curResult.ASIN,mode:'compose',action:action,type: window.amznPs.linkCodesConfigData.imageOnlyProductLink,linkId:linkId};
        window.amznPs.metrics.send(_payLoad);
    }
};

amzn_ps_bm_features.prototype.insertImageOnlyPL = function(curResult, editData) {
    var self = this;
    var $ = amznpubstudio_jQuery;
    var asin = curResult.ASIN;

    var imageUrl = window.amznPs.bootStrap.productImageUrl.replace("__ASIN__", asin);
    var linkcode = window.amznPs.linkCodesConfigData.imageOnlyProductLink;
    var _bootStrap = window.amznPs.bootStrap;
    ...
    var landingPageUrl = curResult.DetailPageURL+"/ref="+window.amznPs.refTagsConfigData.imageOnlyProductLink+"/?tag="+_bootStrap.tag
        + "&linkId=" + linkId
        + "&linkCode=" + linkcode;
    var _bootStrap = window.amznPs.bootStrap;
    var id = Math.ceil(Math.random()*10000000);

    var href = window.amznPs.link.changeLinkProtocol(landingPageUrl,'http:');

    var iPLStr = '<a rel="nofollow" data-amzn-ps-bm-keyword="' + curResult.editHotSpot.searchTerm +
        '" amzn-ps-bm-asin="' + asin +
        '" class="amzn_ps_bm_ipl' +
        '" id="amznPsBmLink_' + id +
        '" href="' + href + '" target="_blank">' +
        '<img src="' + imageUrl +
        '" class="amzn_ps_bm_ipl_image" style="border:0px;" />' +
        '</a>';

    var imgPixel = window.amznPs.metrics.createIPLPixel(id, linkId);

    var modeType = window.amznPs.link.returnModeType();
    if(!modeType || modeType.length==0) {
        logger.log('do not insert PL because the mode-type is not clear');
    }
    else if(modeType=='html') {
        self.insertLinkInTextArea(iPLStr+imgPixel);
        window.amznPs.notifications.showSuccessNotification(window.amznPs.strings.get('ps-bk-image-only-link-success-notification'));
        var _payLoad = {ASIN:asin,mode:'html',action:'create',type: window.amznPs.linkCodesConfigData.imageOnlyProductLink,linkId:linkId};
        window.amznPs.metrics.send(_payLoad);
    }
    else if(modeType=='compose') {
        var iPL = $(iPLStr+imgPixel);
        self.insertLinkInCompose(iPL, 'imageOnlyProductLink', curResult, editData,linkId);
    }
};

amzn_ps_bm_features.prototype.openCopyToClipBoard = function(curResult) {
    var self = this;
    var $ = amznpubstudio_jQuery;

    var url=curResult.DetailPageURL;
    var asin = curResult.ASIN;
    var _bootStrap = window.amznPs.bootStrap;
    var linkcode = window.amznPs.linkCodesConfigData.copyToClipBoard;
    ...
    url=url+"/ref="+window.amznPs.refTagsConfigData.copyToClipBoard+"/?tag="+_bootStrap.tag;
    url += "&linkId=" + linkId
    url += "&linkCode=" + linkcode;
    var urlToCopy=escape(url);
    logger.log(url + ' ' + urlToCopy);

    var $dialog = $("<div/>",{
        id:"copy-to-clbrd"
    }).appendTo($('body'));
    var $overlay = $("<div/>",{
    	'class': 'amznps_overlay'
    }).appendTo($('body'));

    var $S = window.amznPs.strings;

    var close = $("<div/>",{
        id: "amzn-copy-clpbrd-close-btn",
        css: {
            "background": 'url("' + config.sprite1 + '")' + ' -374px -137px repeat-x transparent'
        },
        title: $S.get('ps-close'),
        click: function(e) {
        	  e.preventDefault();
            try {
                $overlay.remove();
    ...
    var $title=$('<p/>',{
        id:"copy-to-clbrd-title",
        text: $S.get('ps-bk-copy-overlay-heading')
    }).appendTo($dialog);
    var $link = $("<textarea/>",{
        id:"copy-to-clbrd-text",
        text:url,
        amznasin:asin,
        css:{
    ...
    }).attr('readonly', true).appendTo($dialog);

    $('<div id="amzn_ps_bm_copy_button"><embed allowscriptaccess="always" flashvars="txtToCopy='+urlToCopy+'" ' +
        'src=' + window.amznPs.bootStrap.swfUrl + ' width="51" height="22">' +
        '<span id="ctc-copied-msg">' + $S.get('ps-bk-copy-success-msg') + '</span></div>').appendTo($dialog);

    $overlay.show();
};

amzn_ps_bm_features.prototype.insertPL = function(curResult, editData) {
    var self = this;
    var $ = amznpubstudio_jQuery;
    var _bootStrap = window.amznPs.bootStrap;
    ...
    // generate link id here
    var linkIdData = {
        tag: _bootStrap.tag,
        marketplaceId: config.marketplaceConfig[curResult.Marketplace].id,
        linkCode: window.amznPs.linkCodesConfigData.productLink,
        toolCreation: 'PubStudio',
        adUnitType: 'TEXT+IMAGE',
        destinationType: 'ASIN',
        asin: curResult.ASIN
    };
    var linkId = AmznLinkID.get(linkIdData, config.stage);

    var url= _bootStrap.productLinkUrl.replace("__ASIN__", asin).replace("__TAG__", _bootStrap.tag);

    url=url.replace('__REF_TAG__', window.amznPs.refTagsConfigData.productLink);
    url=url.replace('__LINK_ID__', linkId);
    
    var id = Math.ceil(Math.random()*10000000);
    var pLStr = '<iframe src="' + url + '" id="amznPsBmLink_' + id +
        '" data-amzn-ps-bm-keyword="' + curResult.editHotSpot.searchTerm +
        '" amzn-ps-bm-asin="' + asin +
        '" class="amzn_ps_bm_pl" style="width:120px;height:240px;' +
    ...
    if(!modeType || modeType.length==0) {
        ...
        self.insertLinkInTextArea(pLStr);
        window.amznPs.notifications.showSuccessNotification(window.amznPs.strings.get('ps-bk-product-link-success-notification'));
        var _payLoad = {ASIN:asin,mode:'html',action:'create',type: window.amznPs.linkCodesConfigData.productLink,linkId:linkId};
        window.amznPs.metrics.send(_payLoad);
    }
    else if(modeType=='compose') {
        var pL = $(pLStr);
        self.insertLinkInCompose(pL, 'productLink', curResult, editData,linkId);
    }
};
amzn_ps_bm_features.prototype.scrollIntoView = function($element,$container) {

    var containerTop = $container.scrollTop();
    var topDiff = $element.offset().top - $container.offset().top - $container.height() + $element.height(); // absolute difference in top position of element and parent
    if(topDiff > 0) { // is the element visible?
        $container.animate({scrollTop:( $container.scrollTop() + topDiff + $element.height() + 2)}); // adding a padding of 2px
    }
};
amzn_ps_bm_features.prototype.init = function() {
    var self = this;
    var $ = amznpubstudio_jQuery;

    window.amznPs.link = new amzn_ps_bm_link();
    window.amznPs.link.init();
};

window.copyToClipboard = function() {
    var $ = amznpubstudio_jQuery;
    var textArea = $('textarea#copy-to-clbrd-text');
    var asin = textArea.attr('amznasin');

    $('span#ctc-copied-msg').show();
    textArea.select();

    var _payLoad = {ASIN:asin,type: window.amznPs.linkCodesConfigData.copyToClipBoard};
    window.amznPs.metrics.send(_payLoad);
};
...
var ccCode = "\
    window.copyToClipboard = function() {\
        var msg = document.querySelector('span#ctc-copied-msg');\
        msg.style.display = 'block';\
    }\
";

try {
    ccScript.appendChild(document.createTextNode(ccCode));
    document.body.appendChild(ccScript);
} catch (e) {
    ccScript.text = ccCode;
    document.body.appendChild(ccScript);
}
 ...
 */
// short code module
function amzn_ps_bm_shortcode() {

}

amzn_ps_bm_shortcode.prototype.getLastShortCodeChunk = function(search){
    // writing custom code for string search, to optimize the operation
    var end = search.length - 1 ;
    var index = -1;
    var prev = '';
    while(end >= 0){
        var cur = search.charAt(end);
        if(prev === '$'){
            if(cur === '$') {
                index = end;
            }else break;
        }
        prev = search.charAt(end);
        end--;
    }
    return index;
};

amzn_ps_bm_shortcode.prototype.addToFBXhpc = function(value){
    var $ = amznpubstudio_jQuery;
    $('input.mentionsHidden').attr('value',value);
};

amzn_ps_bm_shortcode.prototype.addShortCodeLink = function(el,curResult,targetNode) {
    var self  = this;
    el.attr('data-short-code-event',null);
    el.unbind('keydown',self.keyDownHandler);
    el.unbind('keyup',self.getCode);
    el = el.get(0);
    var $ =amznpubstudio_jQuery;
    self.closeSuggestionBox();

    var _bootStrap = window.amznPs.bootStrap;
    var linkcode = window.amznPs.linkCodesConfigData.socialShare;
    ...
    var landingPageUrl = curResult.DetailPageURL+"/ref="+window.amznPs.refTagsConfigData.textLink+"/?tag="+_bootStrap.tag+
        "&linkId=" + linkId + "&linkCode=" + linkcode;

    var shortUrlCallback = function(el, landingPageUrl, targetNode, self, resp) {
        var $ = amznpubstudio_jQuery;
        if(resp && resp.isOk) {
            landingPageUrl = resp.shortUrl;
        }
        if(el.nodeName.toUpperCase() == 'TEXTAREA' || el.nodeName.toUpperCase() == 'INPUT') {
            var val = el.value.substring(0,targetNode.start) + landingPageUrl + el.value.substring(targetNode.end);
            if(resp) {
                // hit enter only when there is a response
                val += '\n';
            }
            el.value = val;
            targetNode.end  = targetNode.start + landingPageUrl.length;
            self.addToFBXhpc(el.value);
            linkingUtils.keyDown(el);
        }
        else if(el.nodeName.toUpperCase() == 'DIV') {
            var _data = targetNode.node.data.substring(0,targetNode.start) + landingPageUrl + targetNode.node.data.substring(targetNode.end);
            var _textNode = DOCUMENT.createTextNode(_data);
            // check if the targetNode is present in the div
            if($(targetNode.node).closest($(el)).length > 0) {
                $(_textNode).insertBefore(targetNode.node);
                targetNode.node.remove();
            } else {
                // most likely got replaced with <a/> tag
                $(el).find('a[href="' + targetNode.url + '"]').replaceWith(landingPageUrl);
                // any other modifications, fail silently
            }

            targetNode.node = _textNode;
            targetNode.end  = targetNode.start + landingPageUrl.length;
            targetNode.url  = landingPageUrl;
        }
    };

    // inject the longURL first to avoid hanging experience for customers
    shortUrlCallback(el, landingPageUrl, targetNode, self);

    if(window.amznPs.options.configFlags.shortenLinks) {
        var requestObj = {
            url: constants.getShortUrl.replace('__long__url__', encodeURIComponent(landingPageUrl)),
            methodType: 'GET',
            data: '',
            timeout: 5000,
            cache: true,
            successCB: shortUrlCallback,
            failureCB: shortUrlCallback,
            successCBArgs: [el, landingPageUrl, targetNode, self],
            failureCBArgs: [el, landingPageUrl, targetNode, self],
            dataType: 'json'
        };
        window.amznPs.xhr.makeXHR(requestObj);
    }
};
amzn_ps_bm_shortcode.prototype.checkShortCode = function(e){
    if(!window.amznPs.bootStrap.isLinkingEnabled()) return;
    var $=amznpubstudio_jQuery;
    var self =e.data.self;
    var ele = $(e.currentTarget);
    if(ele.attr('data-short-code-event') == 'attached' || (typeof ele.attr('data-reactid') != 'undefined')) return;
    var _offset = utils.getCaretOffset(e.currentTarget, window);
    var text = _offset.text;
    if (text && text.length > 0) {
        var _search = text.substring(0, _offset.location);
        var _start = self.getLastShortCodeChunk(_search);
        if ( _start < 0 ) {
            self.closeSuggestionBox();
            return;
        }
        // attach events to target div, if not attached
        ele.attr('data-short-code-event','attached');
        ele.unbind('keydown',self.keyDownHandler);
        ele.unbind('keyup',self.getCode);
        ele.bind('keydown',{self:self,ele:ele},self.keyDownHandler);
        ele.bind('keyup',{self:self,ele:ele},self.getCode);
    }
};
amzn_ps_bm_shortcode.prototype.getCode = function(e){
    if(!window.amznPs.bootStrap.isLinkingEnabled()) return;
    var $=amznpubstudio_jQuery;
    var self =e.data.self;
    var ele = e.data.ele;
    var _offset = utils.getCaretOffset(e.data.ele.get(0), window);
    var text = _offset.text;
    if (text && text.length > 0) {
        var _search = text.substring(0, _offset.location);
        var _start = self.getLastShortCodeChunk(_search);
        if (_start < 0 || _start+2 === _search.length) {
            self.closeSuggestionBox();
            return;
        }
        e.preventDefault();
        e.stopImmediatePropagation();
        _search = _search.substring(_start + 2);
        searchbox.showSuggestionsBox(ele, _search, {node: _offset.element, start: _start, end: _offset.location, 'code': e.which});
    }
};

amzn_ps_bm_shortcode.prototype.keyDownHandler = function (e) {
    if(!window.amznPs.bootStrap.isLinkingEnabled()) return;
    var self =e.data.self;
    if (e.keyCode == 38) {
        e.preventDefault();
...
amzn_ps_bm_shortcode.prototype.attachShortCodeEventsImpl = function() {
    var $=amznpubstudio_jQuery;
    var self=this;
    var notLoggedInMsgTimer = null;
    $(DOCUMENT).ready(function() {
        var $doc = $(DOCUMENT);
        var _selector = 'textarea, div.editable, input[type="text"],div[contenteditable="true"]';
        $doc.undelegate(_selector,'keyup',self.checkShortCode);
        $doc.delegate(_selector,'keyup',{self:self},self.checkShortCode);
    });
};

amzn_ps_bm_shortcode.prototype.deattachShortCodeEventsImpl = function() {
    var $=amznpubstudio_jQuery;
    var self=this;
    var notLoggedInMsgTimer = null;
    var $doc = $(DOCUMENT);
    var _selector = 'textarea, div.editable, input[type="text"],div[contenteditable="true"]';
    $doc.undelegate(_selector,'keyup',self.checkShortCode);
    $doc.undelegate(_selector,'keyup',{self:self},self.checkShortCode);
};

amzn_ps_bm_shortcode.prototype.closeSuggestionBox = function(){
    var $=amznpubstudio_jQuery;
    $("#amznps_adminimgtools").remove();
};

amzn_ps_bm_shortcode.prototype.init = function(shortCodeInfo) {
    if(shortCodeInfo && shortCodeInfo.enabled){
        window.amznPs.shortCode.attachShortCodeEventsImpl();
        window.amznPs.bootStrap.platformEnabled = true;
    }
    else{
        window.amznPs.shortCode.deattachShortCodeEventsImpl();
        window.amznPs.shortCode.closeSuggestionBox();
        window.amznPs.bootStrap.platformEnabled = false;
    }
};
/**
 * Created by misanand on 5/7/14.
...
function amzn_ps_bm_bootstrap() {
    var self = this;
    self.localComposeConfigsKey = 'platform_configs_compose';
    self.localHtmlConfigsKey = 'platform_configs_html';
    self.amznSpecificClass = { 'amznps-short-code' : true };
    self.urlHashMap = {};
    self.tag = undefined;
    var linkCreation = true;
    self.setLinkCreation = function(_linkCreation) {
        linkCreation = _linkCreation;
        //window.amznPs.link.initLinking();
        if(!linkCreation) {
            window.amznPs.link.deAttachTextLinkEventToCompose();
            window.amznPs.link.deAttachTextLinkEventToHtml();
            ...
            window.amznPs.link.attachTextLinkEventToCompose();
            window.amznPs.link.attachTextLinkEventToHtml();
        }
    };
    self.isLinkingEnabled = function(){
        return (typeof self.tag != 'undefined') && linkCreation && window.amznPs.bootStrap.platformEnabled;
    };
}

amzn_ps_bm_bootstrap.prototype.getCurrentUrl = function() {
    /*var locationObj = window.location;
     var url = locationObj.host + locationObj.pathname;
     */
    return window.location.href;
};

amzn_ps_bm_bootstrap.prototype.getHash = function(url){
    var urlHash = 0;
...
amzn_ps_bm_bootstrap.prototype.generateUrlHash = function() {
    var self = this;
    var logger = window.amznPs.logger;

    var url = self.getCurrentUrl();
    if(!self.urlHashMap[url]) {
        var urlHash = self.getHash(url);
        self.urlHashMap[url] = Math.abs(urlHash);
        logger.log("Calculating hash of Url: " + url + ", hash: " + self.urlHashMap[url] );
    } else {
        logger.log("Reusing precalculation hash of Url: " + window.location.host +  window.location.pathname + ", hash: " + self.urlHashMap[url]);
    }

    return self.urlHashMap[url];
};

amzn_ps_bm_bootstrap.prototype.updateTag = function(tag) {
    var self = this;
    ...
    if(tag) {
        ...
        window.amznPs.browser.getPlatformConfigs('aps-store-tag-map', function(value) {
            if(value && value.tag) {
                window.amznPs.bootStrap.tag = value.tag;
            }
        });
    }
};

amzn_ps_bm_bootstrap.prototype.runLinking = function() {
    linkingUtils.closeAdminTools();

    var self = this;

    if(window.amznPs.searchCategories.categoriesInitialized &&
        window.amznPs.strings.stringsInitialized && (this.composeBody || this.htmlTextArea)) {
        //invoke textSelection watch, it opens search-box with keyword populated
        window.amznPs.link.initLinking();
    }
};

amzn_ps_bm_bootstrap.prototype.init = function(cb) {
    var self = this;
    var $ = amznpubstudio_jQuery;

    window.amznPs.constants = constants;

    window.amznPs.browser = new amzn_ps_bm_browser();
    window.amznPs.utils = utils;
    window.amznPs.browser.init(utils);
    window.amznPs.browser.onUrlChange();

    self.updateTag();

    window.amznPs.xhr = xhr;
    window.amznPs.logger = logger;
    window.amznPs.notifications = notifications;
    window.amznPs.strings = strings;

    //window.amznPs.categoryBox = new amzn_ps_bm_category();

    window.amznPs.link = new amzn_ps_bm_link();
    searchbox = new amzn_ps_bm_search();
    window.amznPs.searchbox = searchbox;
    searchbox.init();

    window.amznPs.searchCategories = searchCategories;
    searchCategories.init();
    window.amznPs.metrics = new amzn_ps_bm_metrics(config, constants);
    self.initUserAgentFlags();
    if( typeof cb == 'function' ) cb();
    window.amznPs.bookmarkletFeatures = new amzn_ps_bm_features();
    window.amznPs.bookmarkletFeatures.init();

    window.amznPs.shortCode = new amzn_ps_bm_shortcode();
    window.amznPs.shortCode.attachShortCodeEventsImpl();
    window.amznPs.bootStrap.platformEnabled = true;
    /*
    if(window.amznPs.options.shortCodeInfo && window.amznPs.options.shortCodeInfo.enabled){
        window.amznPs.shortCode.attachShortCodeEventsImpl();
        window.amznPs.bootStrap.platformEnabled = true;
    }
    else{
    ...
    // start buildSearchBox timer, builds it only after strings and categories are complete
    //see if jQuery when api can be used
};


amzn_ps_bm_bootstrap.prototype.getCMS = function() {
    var url = window.location.href;
    if(url.indexOf("blogger.g") !== -1) return "blogger";
    if(url.indexOf("wp-admin") !== -1) return "wordpress";
    if(url.indexOf("option=com_content") !== -1) return "joomla!";
    if(url.indexOf("q=node") !== -1) return "drupal";
    if(url.indexOf("tumblr.com/edit") !== -1) return "tumblr";
    return "N/A";
};

amzn_ps_bm_bootstrap.prototype.initUserAgentFlags = function() {
    var self = this;
    var $ = amznpubstudio_jQuery;

    self.opera = $.browser.opera;
    self.ff = $.browser.mozilla;
    self.safari = Object.prototype.toString.call(window.HTMLElement).indexOf('Constructor') > 0;
    self.chrome = !this.safari && $.browser.webkit;
    self.ie = $.browser.msie;

    var userAgent = navigator.userAgent.toLocaleLowerCase();

    self.isMobileAgent = false;
    self.isTabletAgent = false;
    if( /iphone/i.test(userAgent) || /ipod/i.test(userAgent) ||
        /windows phone/i.test(userAgent) || /symbian/i.test(userAgent) ||
        (/blackberry/i.test(userAgent) && /mobile/i.test(userAgent)) ||
        (/android/i.test(userAgent) && /mobile/i.test(userAgent)) ) {

        self.isMobileAgent = true;
    }
    else if( /ipad/i.test(userAgent) || /kindle/i.test(userAgent) ||
        (/blackberry/i.test(userAgent) && !/mobile/i.test(userAgent)) ||
        (/android/i.test(userAgent) && !/mobile/i.test(userAgent)) ||
        /tablet/i.test(userAgent)) {

        self.isTabletAgent = true;
    }

    self.isTouchAgent = !!('ontouchstart' in window);
};

//util function - makes it easy to toggle all data read from cookies or localStorage
amzn_ps_bm_bootstrap.prototype.getData = function(name) {
    if(this.useLocalStorage && typeof(Storage) != undefined) // Use localstorage
    {
        window.amznPs.logger.log("Reading key: " + name + " from localStorage");
        return localStorage[name];
    } else { // Use cookies
        window.amznPs.logger.log("Reading key: " + name + " from cookies");
        return this.getCookie(name);
    }
};

//util function - makes it easy to toggle all data writes to cookies or localStorage
amzn_ps_bm_bootstrap.prototype.setData = function(name, value, days) {
    if(this.useLocalStorage && typeof(Storage) != undefined) // Use localstorage
    {
        window.amznPs.logger.log("Setting " + name + ":" + value + " in localStorage");
...
//copy pasta cookie code
amzn_ps_bm_bootstrap.prototype.setCookie = function(name,value,days) {
    if (days) {
        var date = new Date();
...
amzn_ps_bm_bootstrap.prototype.getCookie = function(name) {
    var nameEQ = name + "=";
...
amzn_ps_bm_bootstrap.prototype.setComposePlatform = function(obj,callback) {
    var self = this;
    var $ = amznpubstudio_jQuery;

    window.amznPs.logger.log('Compose-mode successfully initialized, unbinding composeComponentsListener');
    $(DOCUMENT).unbind('click', self.composeListener);
    self.composeIframe = obj.composeIframe;
    self.composeWindow = obj.composeWindow;
    self.composeBody = obj.composeBody;
    var _platform = 'N/A';
    if( obj.platform ) _platform = obj.platform;
    else _platform ="fallback";
    self.platform = obj.platform = _platform;
    window.amznPs.link.attachTextLinkEventToCompose(window.amznPs.link.initLinking);
    if(callback) callback();
};


amzn_ps_bm_bootstrap.prototype.setHTMLPlatform = function(obj,callback) {
    var self = this;
    var $ = amznpubstudio_jQuery;
    window.amznPs.logger.log('Html-mode successfully initialized, unbinding htmlComponentsListener');
    $(DOCUMENT).unbind('click', self.htmlListener);
    self.htmlIframe=obj.htmlIframe;
    self.htmlWindow=obj.htmlWindow;
    self.htmlTextArea=obj.htmlTextArea;
    var _platform = 'N/A';
    if( obj.platform ) _platform = obj.platform;
    else _platform = self.getCMS();
    self.platform = obj.platform = _platform;
    window.amznPs.link.attachTextLinkEventToHtml(window.amznPs.link.initLinking);
    if(callback) callback();
};
amzn_ps_bm_bootstrap.prototype.setPlatform = function(data){
    var self = window.amznPs.bootStrap;
    var $ = amznpubstudio_jQuery;
    self.platformEnabled = true;

    if(data['compose']) {
        if(!data['compose'].enabled) {
            self.platformEnabled = false;
        }

        if(!self.composeBody) {
            var composePlt = data['compose'];
            var $composeIfr = $(composePlt['composeIframeId']);
            if($composeIfr && $composeIfr.length>0) {
                var plt = {composeIframe:$composeIfr,composeWindow: $composeIfr.get(0).contentWindow,composeBody : $composeIfr.contents().find("body")};
                self.setComposePlatform(plt);
            }
        }
        else {
            window.amznPs.bootStrap.setLinkCreation(self.platformEnabled);
        }
    }

    if(data['html']) {
        if(!data['html'].enabled) {
            self.platformEnabled = false;
        }

        if(!self.htmlTextArea) {
            var htmlPlt = data['html'];
            var plt = {htmlIframe: null};
            if(htmlPlt.htmlIframeId) {
                var iframe = $(htmlPlt.htmlIframeId);
                if(iframe && iframe.length>0) {
                    plt['htmlIframe'] = iframe;
                    plt['htmlWindow'] =plt.htmlIframe.get(0).contentWindow;
                    plt['htmlTextArea'] =$(plt.htmlIframe.contents().find(htmlPlt.htmlTextAreaId));
                    self.setHTMLPlatform(plt);
                }
            }
            else{
                plt['htmlWindow'] =window;
                var ta = $(htmlPlt.htmlTextAreaId);
                if(ta && ta.length>0) {
                    plt['htmlTextArea'] = ta;
                    self.setHTMLPlatform(plt);
                }
...
//todo use only 1 local-variable, right now used to keep common searchbox.js
if(typeof bootstrap == 'undefined') var bootstrap = null;

function amznPsBmLaunchApp (command, data ,cb) {
    var _init = function(cb) {
        if(!window.amznPs.bootStrap) {
            window.amznPs.bootStrap = new amzn_ps_bm_bootstrap();
            bootstrap = window.amznPs.bootStrap;
            window.amznPs.bootStrap.init(cb);
        }
    ...
    var _runApp = function() {
        if(typeof window.amznPs != 'undefined') {
            if(typeof window.amznPs.shortCode != 'undefined') window.amznPs.shortCode.attachShortCodeEventsImpl();
            if(typeof window.amznPs.bootStrap != 'undefined') window.amznPs.bootStrap.runLinking();
        }
    };
    var _setConfigs = function(data) {
        var cb = function(data){
            return function(){
                window.amznPs.bootStrap.setPlatform(data);
            };
        }(data.platform);
        _init(cb);
    };
    ...
    if( typeof _window[command] == 'function') {
        _window[command](data);
    }
    if(typeof cb == 'function') cb();

}
var scriptName = "creation-content";
amznPsBmLaunchApp("init");